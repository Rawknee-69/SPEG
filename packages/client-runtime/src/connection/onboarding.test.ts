import { AuthStandardClientScopes, EnvironmentId } from "@speg/contracts";
...
function pairingHttpLayer(
  ...
  const fetchFn = ((input, init = {}) => {
    ...
    if (url.endsWith("/.well-known/speg/environment")) {
  ...
  it.effect("prepares a persisted bearer registration from pairing details", () =>
    Effect.gen(function* () {
      ...
      expect(calls.map((call) => call.url)).toEqual([
        "https://remote.example.test/.well-known/speg/environment",
      ...
      expect(tokenParams.get("client_label")).toBe("SPEG Test");
    }),
  );
