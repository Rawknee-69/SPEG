...
import { classifyTaskAgentKind, type OrchestrationThreadActivity } from "@speg/contracts";
import {
...
describe("terminal robustness", () => {
  ...
  it("provider endedAt wins over ingestion time on the settling transition", () => {
    const agents = fold([
      activity("task.started", { taskId: "speg", taskType: "local_agent" }),
      activity(
        "task.updated",
        { taskId: "speg", status: "failed", endedAt: "2026-08-01T09:59:59.000Z" },
