    ...
    it.effect("flushes buffered trace records on close", () =>
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const tempDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "speg-trace-sink-" });
    ...
    it.effect("writes nested spans to disk and captures log messages as span events", () =>
      Effect.scoped(
        Effect.gen(function* () {
          const fileSystem = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          const tempDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "speg-local-tracer-" });
          const tracePath = path.join(tempDir, "shared.trace.ndjson");
