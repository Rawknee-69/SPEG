...
import { resolveGitWorktreePath, resolveWorktreeSpegHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "speg-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeSpegHome", () => {
  it.effect("answers with .speg before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeSpegHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".speg"));
