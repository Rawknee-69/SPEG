...
import { buildSpegProjectFileJsonSchema, SpegProjectFileFromJson } from "./spegProjectFile.ts";

const decodeJson = Schema.decodeUnknownSync(SpegProjectFileFromJson);

describe("buildSpegProjectFileJsonSchema", () => {
  it("emits a draft 2020-12 schema with the published $id", () => {
    const schema = buildSpegProjectFileJsonSchema();
  ...
  it("documents every supported field", () => {
    const schema = buildSpegProjectFileJsonSchema() as {
      properties: Record<
        string,
        {
          description?: string;
...
describe("SpegProjectFileFromJson", () => {
