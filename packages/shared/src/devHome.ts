 ...
 * `~/.speg` that a user's installed SPEG runs against.
 *
 * A linked git worktree gets its own (gitignored) `.speg`: feature work in a
 * throwaway branch must not share a database with the real app, and an ambient
 * `SPEG_HOME` counts as an explicit base dir — flipping the state directory
 ...
 */
export const resolveWorktreeSpegHome = (
  cwd: string,
): Effect.Effect<string | undefined, never, FileSystem.FileSystem | Path.Path> =>
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, ".speg");
  });
