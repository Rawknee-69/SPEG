...
}).annotate({ description: "Links an authenticated cloud user to a SPEG environment." });
...
>()("@speg/contracts/relay/RelayClientPrincipal") {}
...
>()("@speg/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in SPEG Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["speg-mobile", "speg-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "speg-mobile" as const;
export const RelayWebClientId = "speg-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "SPEG Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking SPEG environments, connecting authorized clients, and publishing agent activity.",
  );
