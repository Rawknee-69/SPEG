...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "speg-ssh-askpass-test-" });
      ...
      assert.equal(env.SPEG_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "speg");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\speg-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\speg-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
