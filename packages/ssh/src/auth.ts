...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@speg/ssh/auth/SshPasswordPrompt",
) {
...
const SSH_ASKPASS_DIR_NAME = "speg-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when SPEG re-runs ssh with a cached password
# from the renderer's in-app prompt. We never expose a native dialog here - if
# SPEG_SSH_AUTH_SECRET is missing, that's a caller bug and we fail loudly.
if [ "\${SPEG_SSH_AUTH_SECRET+x}" = "x" ]; then
  printf "%s\\n" "$SPEG_SSH_AUTH_SECRET"
  exit 0
fi
printf 'SPEG ssh-askpass invoked without SPEG_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when SPEG re-runs\r
# ssh with a cached password from the renderer's in-app prompt. We never expose\r
# a native dialog here - if SPEG_SSH_AUTH_SECRET is missing, that's a caller bug\r
# and we fail loudly.\r
if ($null -ne $env:SPEG_SSH_AUTH_SECRET) {\r
  [Console]::Out.WriteLine($env:SPEG_SSH_AUTH_SECRET)\r
  exit 0\r
}\r
[Console]::Error.WriteLine("SPEG ssh-askpass invoked without SPEG_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "speg-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(input.authSecret === undefined ? {} : { SPEG_SSH_AUTH_SECRET: input.authSecret ?? "" }),
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "speg" }),
  };
});
