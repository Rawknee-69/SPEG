...
describe("ssh tunnel scripts", () => {
  it("builds the remote speg runner with npx and npm fallbacks", () => {
    const script = buildRemoteSpegRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "SPEG_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec speg "$@"');
    assert.include(script, "exec npx --yes 'speg@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'speg@latest' -- \"$@\"");
    assert.include(script, "could not install 'speg@latest'");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `SPEG_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for SPEG_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteSpegRunnerScript();

    assert.include(script, "SPEG_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote speg runner", () => {
    const script = buildRemoteSpegRunnerScript({
      packageSpec: "speg@nightly; touch /tmp/speg-owned",
    });

    assert.include(script, "exec npx --yes 'speg@nightly; touch /tmp/speg-owned' \"$@\"");
    assert.include(script, "exec npm exec --yes 'speg@nightly; touch /tmp/speg-owned' -- \"$@\"");
    assert.notInclude(script, "exec npx --yes speg@nightly; touch /tmp/speg-owned");
  });

  it("builds the remote speg runner with a node script override", () => {
    const script = buildRemoteSpegRunnerScript({
    ...
    assert.include(
      script,
      "SPEG_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$SPEG_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote speg runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `SPEG_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote SPEG server did not become ready");
    assert.include(buildRemoteLaunchScript({ packageSpec: "speg@nightly" }), "speg@nightly");
    assert.include(
    ...
    assert.include(buildRemotePairingScript(target, { packageSpec: "speg@nightly" }), "speg@nightly");
    assert.include(
