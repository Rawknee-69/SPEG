...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@speg/contracts";
...
const PUBLISHABLE_SPEG_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteSpegCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_SPEG_VERSION_PATTERN.test(appVersion)) {
    return `speg@${appVersion}`;
  }
  ...
  return input.updateChannel === "nightly" ? "speg@nightly" : "speg@latest";
}
