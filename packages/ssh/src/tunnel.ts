...
} from "@speg/shared/httpReadiness";
import * as NetService from "@speg/shared/Net";
import { extractJsonObject, fromLenientJson } from "@speg/shared/schemaJson";
import { satisfiesSemverRange } from "@speg/shared/semver";
...
export interface RemoteSpegRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteSpegRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteSpegRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  SPEG_NODE_ENGINE_RANGE=@@SPEG_NODE_ENGINE_RANGE@@
  if [ -z "$SPEG_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$SPEG_NODE_ENGINE_RANGE" <<'NODE'
@@SPEG_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for SPEG_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$SPEG_NODE_BIN/node" ]; then
        PATH="$SPEG_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@SPEG_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
SPEG_NODE_SCRIPT_PATH=@@SPEG_NODE_SCRIPT_PATH@@
if [ -n "$SPEG_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$SPEG_NODE_SCRIPT_PATH" "$@"
fi
if command -v speg >/dev/null 2>&1; then
  exec speg "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@SPEG_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@SPEG_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the speg CLI and could not install @@SPEG_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@SPEG_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.speg/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.speg"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-speg.sh"
RUNNER_NEXT="$STATE_DIR/run-speg.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@SPEG_DEFAULT_REMOTE_PORT@@" "@@SPEG_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@SPEG_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@SPEG_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@SPEG_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@SPEG_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@SPEG_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@SPEG_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env SPEG_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@SPEG_READY_TIMEOUT_MS@@"; then
    printf 'Remote SPEG server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.speg/ssh-launch/@@SPEG_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.speg"
RUNNER_FILE="$STATE_DIR/run-speg.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteSpegRunnerScript(input?: RemoteSpegRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "speg@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      SPEG_PACKAGE_SPEC: packageSpec,
      SPEG_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      SPEG_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemoteSpegRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      SPEG_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      SPEG_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteSpegRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    SPEG_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    SPEG_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteSpegRunnerScript(input)),
    SPEG_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    SPEG_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    SPEG_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    SPEG_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    SPEG_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    SPEG_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    SPEG_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteSpegRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    SPEG_STATE_KEY: remoteStateKey(target),
    SPEG_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteSpegRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteSpegRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteSpegRunnerOptions;
...
>()("@speg/ssh/tunnel/SshEnvironmentManager") {
