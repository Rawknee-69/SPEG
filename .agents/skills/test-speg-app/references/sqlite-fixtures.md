...
Load this reference only when inspecting or seeding local SPEG state directly.
...
```bash
node apps/server/scripts/speg-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/speg-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/speg-seed.sql
```
