---
name: test-speg-mobile
description: Launch and test SPEG Mobile on an iOS Simulator or Android Emulator against disposable local SPEG environments, including Metro and dev-client reuse, native rebuild decisions, per-client pairing, seeded projects, semantic UI control, screenshots, and iOS serve-sim streaming. Use after mobile UI or native changes, when reproducing phone or tablet behavior, pairing an emulator to isolated state, or verifying mobile behavior on macOS, Linux, or Windows.
---

# Test SPEG Mobile

Run one focused, end-to-end mobile verification pass against disposable SPEG state. Use the sibling [`test-speg-app`](../test-speg-app/SKILL.md) skill as the detailed reference for pairing-token semantics and SQLite fixtures.
...
- App: `SPEG Dev`
- Bundle/package identifier: `com.speg.speg.dev`
- URL scheme: `speg-dev`
...
## Start one disposable SPEG environment

Run backend commands from the repository root. Use the ignored, worktree-local `.speg` directory or create a fresh directory with the host OS's temporary-directory mechanism. An explicit base directory stores state in `<base-dir>/userdata`; never point testing at shared `~/.speg` state.
...
Use direct SQLite mutation only for disposable projection fixtures. Follow `test-speg-app` and stop the backend before writing.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `speg-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/SpegDev.xcworkspace`
- Scheme: `SpegDev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `com.speg.speg.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.speg.speg.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.speg.speg.dev
...
```bash
SPEG_PORT=<server-port> node apps/server/src/bin.ts auth pairing create \
  --base-dir <base-dir> \
...
In PowerShell, set `$env:SPEG_PORT = "<server-port>"` first and run the `node ... auth pairing create` command without the leading assignment.
...
```bash
xcrun simctl openurl <simulator-udid> 'speg-dev://connections/new'
adb -s <emulator-serial> shell am start -W \
  -a android.intent.action.VIEW \
  -d 'speg-dev://connections/new' \
  com.speg.speg.dev
```

Run only the command for the selected platform.

In SPEG Dev, open Add Environment and enter the complete `<mobile-origin>` and newly printed `Token`. Verify the expected seeded projects appear before exercising the affected flow.
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in SPEG when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from SPEG Dev.
