...
SPEG connects to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving the app.

## Supported Providers

SPEG works with the platforms your team already uses:
...
- SPEG can suggest titles and descriptions based on your commits
...
1. Install the GitHub CLI on the machine running SPEG:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in SPEG and verify GitHub shows as authenticated
...
machine running SPEG.

Recommended, a Bitbucket access token:

```bash
export SPEG_BITBUCKET_ACCESS_TOKEN="your-access-token"
```
...
```bash
export SPEG_BITBUCKET_EMAIL="you@example.com"
export SPEG_BITBUCKET_API_TOKEN="your-token"
```

If both are set, the access token wins. Restart SPEG and verify the connection in **Source
Control settings**.
...
**Git is required** – SPEG uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running SPEG (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
