...
The same configuration lives in `~/.speg/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. SPEG writes the built-in defaults into that file on
