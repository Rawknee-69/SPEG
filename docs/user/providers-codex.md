...
This guide is for people who want to use more than one Codex account in SPEG. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install SPEG](./install.md).
...
- both accounts can see the same SPEG/Codex sessions
...
In SPEG Settings, name it something obvious:
...
In SPEG Settings, add another Codex provider:
...
SPEG shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, SPEG treats it
