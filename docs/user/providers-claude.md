...
This guide is for people who want to use more than one Claude setup in SPEG. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install SPEG](./install.md).
...
In SPEG Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means SPEG uses Claude Code's normal config directory.

When you set this field, SPEG points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where SPEG looks.

Then add another Claude provider in SPEG:
...
SPEG only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so SPEG keeps separate config directories isolated
...
Add or edit a Claude provider in SPEG Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. SPEG stores the value as a server secret and does not
...
SPEG does not need a special Claude Code Router provider. Treat the router as a Claude
