...
Use this when you want to connect to a SPEG server from another device such as a phone, tablet, or separate desktop app.
...
`speg pair` finds the running server (the shared `~/.speg` install, or the current worktree's dev server when run inside one), issues a one-time pairing token, and prints the pairing URL as a QR code you can scan from your phone.
...
If no server is running, `speg pair` says so and points you at `npx speg serve` or `npx speg connect`.
...
run a headless server from the CLI, or have the desktop app launch SPEG over SSH.
...
backend with the same server-side behavior as `speg serve --tailscale-serve`, then the server asks
...
Run the server with `speg serve`.

```bash
npx speg serve --host "$(tailscale ip -4)"
```

`speg serve` starts the server without opening a browser and prints:
...
Use `speg serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

For hosted web pairing over Tailscale HTTPS, opt in to Tailscale Serve:

```bash
npx speg serve --tailscale-serve
```
...
```bash
npx speg serve --tailscale-serve --tailscale-serve-port 8443
```
...
Use this when you want the desktop app to start or reuse SPEG on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote SPEG server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual SPEG server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.speg/ssh-launch/<host-key>/`, starts or reuses a remote SPEG server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. SPEG uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, SPEG first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path SPEG uses:
...
If reconnecting after an app update fails, retry the SSH launch once. The launcher now compares its generated runner script, stops stale launcher-managed remote servers, clears the SSH launch PID/port state, and starts a fresh remote server. You should not normally need to delete `~/.speg/ssh-launch` or kill `speg` processes manually.

## Updating a Remote Server

When the SPEG web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: SPEG may
...
see [Keeping SPEG in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running SPEG in the Background](./background-service.md).
...
1. `speg serve` issues a one-time owner pairing token.
...
Hosted pairing does not proxy traffic through SPEG. The browser still connects directly to the backend URL in the pairing link.

## Managing Access Later

Use `speg auth` to manage access after the initial pairing flow.
...
Use `speg auth --help` and the nested subcommand help pages for the full reference.
...
- Use `speg auth` to revoke credentials or sessions you no longer trust.
