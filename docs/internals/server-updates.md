...
The launcher is the only runtime writer of `service-state.json`. `speg service install` and
`speg service update` may replace the launcher and state while the unit is stopped. Server children
...
1. The active server installs `speg@<target>` into a unique staging directory.
...
```sh
npx speg@<version> service update
```
