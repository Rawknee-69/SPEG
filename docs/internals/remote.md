...
SPEG has one runtime boundary: a client talks to a SPEG server over HTTP and WebSocket, and the server
...
└───────────────┬──────────────────────────────┘
                │ connects to one SPEG server
┌───────────────▼──────────────────────────────┐
│ Execution environment = one SPEG server        │
...
One running SPEG server instance. It owns provider availability and auth, model availability, projects
...
| `RelayConnectionTarget`   | Managed SPEG Connect relay tunnels.                                        |
...
Tailscale is the first provider, and SPEG manages more than discovery. When `tailscaleServeEnabled` is
...
(`apps/server/src/server.ts`, using [`@speg/tailscale`](../../packages/tailscale/src/tailscale.ts)).
...
Access answers one question: how does the client speak WebSocket to a SPEG server? It does not answer
how the server got started or who manages the process.

### Direct WebSocket access

`wss://speg.example.com` or `ws://10.0.0.15:3773`, paired as a bearer target. This is the base model.
...
Managed SPEG Connect relay tunnels use `RelayConnectionTarget` and are the answer when the host is
...
Worker itself. See [speg-connect.md](./speg-connect.md).

### Tailscale access

A SPEG-managed `tailscale serve` mapping exposes the server on the tailnet over HTTPS, and the
...
[packages/ssh/src/tunnel.ts][sshtunnel], which resolves the target, launches or reuses the remote SPEG
...
Launch answers a different question: how does a SPEG server come to exist on the target machine? Keep
it separate from access.

- **Pre-existing server.** The operator already runs SPEG and the client connects directly or through a
  tunnel.
...
- **Client-managed local publish.** A local server is published through the relay with
  `speg connect link`, exposing a desktop-hosted environment to mobile without router or firewall
  changes.
...
- a relay-hosted OAuth callback broker (see [speg-connect.md](./speg-connect.md));
