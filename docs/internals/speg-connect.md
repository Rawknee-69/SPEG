...
> For maintainers. Using SPEG? See [docs/user](../user/).

SPEG Connect uses one Clerk application for web, desktop, and mobile authentication. The relay verifies
two kinds of bearer credential: template JWTs generated from the `speg-relay` template with the shared
`speg-code-relay` audience, and Clerk OAuth tokens issued to the CLI. `verifyRelayClientBearerToken` in
...
[speg-code-connect-auth-flow.html](./speg-code-connect-auth-flow.html).
...
```dotenv
SPEG_CLERK_PUBLISHABLE_KEY=<publishable key>
SPEG_CLERK_JWT_TEMPLATE=<JWT template name>
SPEG_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
SPEG_RELAY_URL=https://relay.example.com
```
...
should set `SPEG_CLERK_PUBLISHABLE_KEY`, `SPEG_CLERK_JWT_TEMPLATE`,
`SPEG_CLERK_CLI_OAUTH_CLIENT_ID`, and `SPEG_RELAY_URL` before building. EAS preview and
production builds only need the Clerk publishable key, JWT template name, and relay URL in their EAS
environment.

When any client-facing public value is absent, cloud UI is omitted. The `speg connect` command group is
...
deployment defaults.
`vp run --filter speg-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `speg connect` commands authorize a headless environment with a separate Clerk OAuth application.
This uses an OAuth public client with PKCE, so the CLI stores no client secret.

In **Clerk Dashboard > OAuth applications**:

1. Create an OAuth application for the SPEG CLI.
...
3. Add **both** allowed redirect URIs:
   ...
   - `https://app.t3.codes/connect/callback` for the hosted out-of-band flow. This is
     ...
     custom `SPEG_HOSTED_APP_URL` means `$SPEG_HOSTED_APP_URL/connect/callback` instead.
     Omitting it breaks headless and SSH authorization.
4. Enable the `openid`, `profile`, and `email` scopes.
5. Set `SPEG_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
```sh
speg connect            # default: onboarding
speg connect login
speg connect link       # --publish-only
speg connect status     # --json
speg connect publish    # --disable
speg connect unlink
speg connect logout
```

`speg serve` is a separate top-level command, not a connect subcommand.

`speg connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `speg connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running SPEG server. The next `speg serve` or `speg start` reconciles the relay link and launches the
managed tunnel. `speg connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `speg connect link` can re-enable exposure without another browser flow. `speg connect
...
logout leaves it running; manage it with `speg service status`, `install`, `update`, and `uninstall`.
...
| ------- | ---------------------------- |
| Name    | `speg-relay`                   |
| Claims  | `{ "aud": "speg-code-relay" }` |

Set `SPEG_CLERK_JWT_TEMPLATE=speg-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=speg-code-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `SPEG_RELAY_URL` still
...
Local desktop development uses `speg-dev://app`, while packaged builds use `speg://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `speg-dev://app`; the production Clerk instance should only need
`speg://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `com.speg.speg`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.speg.speg` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.speg.speg`.
...
```dotenv
SPEG_APPLE_TEAM_ID=ABC1234567
SPEG_MACOS_PROVISIONING_PROFILE=/absolute/path/to/speg.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
SPEG_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `SPEG_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`SPEG_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `SPEG_PORT` set. Rebuild the signed app
...
SPEG_PORT=13773 \
  "/Applications/SPEG (Alpha).app/Contents/MacOS/SPEG (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/SPEG (Alpha).app"
codesign -d --entitlements :- "/Applications/SPEG (Alpha).app"
```
...
Signed-in users manage SPEG Connect under **Connections**. The settings sidebar also has dedicated
controls, rendered by `SettingsSidebarNav.tsx`: `SpegConnectSidebarSignIn` in the footer shows a
**Sign in to SPEG Connect** button while signed out, and `SpegConnectSidebarAvatar` shows a Clerk
