...
SPEG uses [Vite+](https://viteplus.dev/guide/). Install the global `vp` command, install
...
- `vp run dev --share`: Also publishes the web port over HTTPS on this machine's tailnet. The
  ...
  Shared runs default to Vite's bundled dev mode (`SPEG_BUNDLED_DEV=1`): a remote browser pays a
  network round trip per import level in unbundled dev, which turns a cold module graph into
  minutes of waterfall. Set `SPEG_BUNDLED_DEV=0` to opt a shared run back out.
- `vp run dev --browser`: Auto-opens a browser. Off by default. The dev runner writes
  `SPEG_NO_BROWSER` itself from this flag, so setting `SPEG_NO_BROWSER=0` in your environment has
...
- Pass dev-runner flags directly after the root task name, for example:
  `vp run dev --home-dir /tmp/speg-dev`

### Dev state directories

- Dev commands run from a linked **git worktree** default to that worktree's gitignored `.speg`, even
  when `SPEG_HOME` is set, storing state in `<worktree>/.speg/userdata`. Pass `--home-dir <path>` to
  choose another isolated directory explicitly. Submodules are not worktrees and keep the normal
  precedence.
- From the **main checkout**, dev commands implicitly use `~/.speg/dev`, keeping development state
  separate from `~/.speg/userdata`. An explicit `--home-dir <path>` stores state under
  `<path>/userdata`; the base directory remains available for caches, worktrees, and other shared
  data.

## Build, check, test

- `vp run build`: Fans out over `apps/*`, `packages/*`, `oxlint-plugin-speg`, and `scripts`.
...
- `node apps/server/scripts/speg-sqlite-state.ts <query|exec> --base-dir <path> ...`: Inspects or seeds
  an isolated SPEG SQLite database; writes create a private backup first.
...
- Desktop production windows load the bundled UI from the `speg://app/` root URL (not a
  `127.0.0.1` document URL, and not an explicit `index.html` path).
- Desktop packaging includes `apps/server/dist` (the `speg` backend) and starts it on loopback with an
...
- Signed macOS builds also require `SPEG_APPLE_TEAM_ID` and
  `SPEG_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `SPEG_CLERK_PUBLISHABLE_KEY` unless `SPEG_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
- Example: `SPEG_DEV_INSTANCE=branch-a vp run dev:desktop`

Offset resolution, in order:

1. `SPEG_PORT_OFFSET`, which must be a non-negative integer. Negative values are rejected.
2. `SPEG_DEV_INSTANCE`. An all-digit value is used directly as the offset; any other non-empty
