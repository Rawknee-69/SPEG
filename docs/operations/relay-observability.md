...
- `speg-code-relay-traces-prod`, the OpenTelemetry trace dataset shared by the Worker, mobile app, and
  first-party relay clients
- `speg-code-relay-otel-ingest-prod`, the dataset-scoped Worker ingest token
- `speg-code-mobile-otel-ingest-prod`, the dataset-scoped mobile ingest token
- `speg-code-relay-client-otel-ingest-prod`, the dataset-scoped first-party relay-client ingest token
- `speg-code-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`speg-code-relay-traces-dev-julius` for a personal stage.
