...
SPEG has one server-side observability model:
...
stdout/stderr at `~/.speg/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.speg/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.speg/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.speg/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.speg/dev/logs/server.trace.ndjson`.
...
```bash
export SPEG_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export SPEG_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export SPEG_OTLP_SERVICE_NAME=speg-local
```

Optional:

```bash
export SPEG_TRACE_MIN_LEVEL=Info
export SPEG_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `SPEG_OTLP_*`.

macOS app bundle example:

```bash
SPEG_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
SPEG_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
SPEG_OTLP_SERVICE_NAME=speg-desktop \
"/Applications/SPEG.app/Contents/MacOS/SPEG"
```
...
```bash
TRACE_FILE="${SPEG_HOME:-$HOME/.speg}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.speg/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.speg/dev/logs/server.trace.ndjson"
```
...
- service name such as `speg-local`, `speg-dev`, or `speg-desktop`
...
- `speg_rpc_request_duration`
- `speg_orchestration_command_duration`
- `speg_orchestration_command_ack_duration`
- `speg_provider_turn_duration`
- `speg_git_command_duration`

Counters tell you volume and failure rate:

- `speg_rpc_requests_total`
- `speg_orchestration_commands_total`
- `speg_provider_turns_total`
- `speg_git_commands_total`
...
`speg_orchestration_command_ack_duration` measures:
...
1. Check `speg_orchestration_command_ack_duration` by `commandType`.
...
- `SPEG_OTLP_TRACES_URL` was not set
...
- `SPEG_TRACE_FILE`: override trace file path
- `SPEG_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `SPEG_TRACE_MAX_FILES`: rotated file count, default `10`
- `SPEG_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `SPEG_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `SPEG_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `SPEG_OTLP_TRACES_URL`: OTLP trace endpoint
- `SPEG_OTLP_METRICS_URL`: OTLP metric endpoint
- `SPEG_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `SPEG_OTLP_SERVICE_NAME`: service name, default `speg-server`
