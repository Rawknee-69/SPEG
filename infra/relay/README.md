...
> [!NOTE]
> Sign in to SPEG Connect from the app under Settings > Connections.

The relay is the hosted control plane for SPEG Connect. It helps clients discover and connect to
...
The relay is intentionally not in the hot path for normal SPEG traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [SPEG Connect architecture overview](../../docs/internals/speg-code-connect-auth-flow.html) for the larger system
design.
...
- Linking SPEG environments to a cloud account.
...
```sh
vp run --filter speg-relay deploy
```
...
```sh
vp run --filter speg-relay deploy -- --stage prod
vp run --filter speg-relay deploy -- --env-file .env.local
```
...
- [SPEG Connect Clerk Setup](../../docs/internals/speg-connect.md) for Clerk keys, JWT templates, and sign-up restrictions
  setup.
- [Relay Observability](../../docs/operations/relay-observability.md) for deployment tracing and diagnostics.
- [SPEG Connect Architecture Overview](../../docs/internals/speg-code-connect-auth-flow.html) for the full link,
