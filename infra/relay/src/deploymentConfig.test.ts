...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("speg-code-relay-traces", "prod")).toBe(
      "speg-code-relay-traces-prod",
    );
    expect(relayResourceNameForStage("speg-code-relay-traces", "dev_julius")).toBe(
      "speg-code-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "spegrelay.com", hash)).toBe(
      "prod-abcdef0123456789.spegrelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "spegrelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
