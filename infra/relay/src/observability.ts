...
export const RelayObservability = Effect.gen(function* () {
  ...
  const traces = yield* Axiom.Dataset("RelayTracesDataset", {
    name: relayResourceNameForStage("speg-code-relay-traces", stage),
    kind: "otel:traces:v1",
    description: "SPEG relay Worker HTTP request spans.",
    retentionDays: 30,
    useRetentionPeriod: true,
  });

  const workerIngestToken = yield* Axiom.ApiToken("RelayWorkerAxiomIngestToken", {
    name: relayResourceNameForStage("speg-code-relay-otel-ingest", stage),
  ...
  const mobileIngestToken = yield* Axiom.ApiToken("RelayMobileAxiomIngestToken", {
    name: relayResourceNameForStage("speg-code-mobile-otel-ingest", stage),
    description: "Owned by Alchemy. Scoped OTLP ingest token for SPEG mobile spans.",
    datasetCapabilities: Output.map(traces.name, (dataset) => ({
      [dataset]: { ingest: ["create" as const] },
    })),
  });
  ...
  const clientIngestToken = yield* Axiom.ApiToken("RelayClientAxiomIngestToken", {
    name: relayResourceNameForStage("speg-code-relay-client-otel-ingest", stage),
  ...
  yield* Axiom.View("RelayRecentSpansView", {
    name: relayResourceNameForStage("speg-code-relay-recent-spans", stage),
...
export const makeRelayTraceLayer = (input: {
  ...
  Layer.effect(
    ...
    OtlpTracer.make({
      ...
      resource: {
        serviceName: "speg-code-relay-worker",
        attributes: {
