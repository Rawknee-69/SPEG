...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    SPEG_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    SPEG_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    SPEG_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    SPEG_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    SPEG_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    SPEG_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("SPEG_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("SPEG_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "SpegRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the SPEG relay through Alchemy."));
