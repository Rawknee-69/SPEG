...
describe("reconcileRootEnvRelayUrl", () => {
  ...
  it("preserves unrelated root env entries while replacing a previous relay URL", () => {
    expect(
      reconcileRootEnvRelayUrl(
        "SPEG_CLERK_PUBLISHABLE_KEY=pk_test_example\nSPEG_RELAY_URL=https://old.example.test\n",
...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "speg-code-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "speg-code-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "SPEG_MOBILE_OTLP_TRACES_DATASET=speg-code-mobile-traces-dev",
        "SPEG_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "SPEG_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "SPEG_RELAY_CLIENT_OTLP_TRACES_DATASET=speg-code-relay-client-traces-dev",
        "SPEG_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
    ...
    expect(
      reconcileRootEnvPublicConfig(
        [
          "SPEG_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "SPEG_RELAY_URL=https://old.example.test",
          "SPEG_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "SPEG_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "SPEG_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "SPEG_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "SPEG_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "SPEG_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "SPEG_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "SPEG_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "SPEG_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
