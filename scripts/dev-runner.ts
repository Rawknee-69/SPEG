...
import * as NetService from "@speg/shared/Net";
import { resolveGitWorktreePath, resolveWorktreeSpegHome } from "@speg/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@speg/shared/hostProcess";
...
export const DEFAULT_SPEG_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".speg"),
);

const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@speg/contracts",
    "--filter=@speg/web",
    "--filter=speg",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=speg", "dev"],
  "dev:web": ["run", "--filter=@speg/web", "dev"],
  "dev:desktop": ["run", "--filter=@speg/desktop", "--filter=@speg/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`SPEG_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("SPEG_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("SPEG_PORT_OFFSET"),
  devInstance: optionalStringConfig("SPEG_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    ...
    return Effect.succeed({
      ...
      source: `SPEG_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric SPEG_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed SPEG_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_SPEG_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly spegHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .speg > ambient SPEG_HOME) is resolved
    // by the caller; an unset spegHome here genuinely means "use the default".
    const configuredBaseDir = spegHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.SPEG_HOME = resolvedBaseDir;
    } else {
      delete output.SPEG_HOME;
    }

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed speg service (an
    // agent working inside SPEG), these leak through and the child server
    // fails startup with "The service launcher started a different speg version"
    // (serviceLauncherClient.ts resolveStartup).
    delete output.SPEG_SERVICE_LAUNCHER_CONTEXT;
    delete output.SPEG_BOOT_SERVICE_UNIT;

    if (!isDesktopMode) {
      output.SPEG_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.SPEG_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.SPEG_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.SPEG_PORT = String(serverPort);
      ...
      delete output.SPEG_SINGLE_ORIGIN_DEV;
      delete output.SPEG_MODE;
      delete output.SPEG_NO_BROWSER;
      delete output.SPEG_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.SPEG_HOST = host;
    }

    if (!isDesktopMode) {
      output.SPEG_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.SPEG_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.SPEG_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.SPEG_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.SPEG_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.SPEG_MODE = "web";
      delete output.SPEG_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`SPEG_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["SPEG_PORT_OFFSET", "SPEG_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.speg` — see @speg/shared/devHome for why this must
    // outrank an ambient SPEG_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreeSpegHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedSpegHome =
      (input.spegHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.SPEG_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      spegHome: resolvedSpegHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.SPEG_HOME ?? (yield* DEFAULT_SPEG_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.SPEG_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          // a few chunk requests. Only defaulted, so SPEG_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.SPEG_BUNDLED_DEV === undefined) {
            env.SPEG_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  spegHome: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit SPEG data directory; runtime state is stored under userdata (equivalent to SPEG_HOME). Inside a git worktree this defaults to that worktree's own .speg so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to SPEG_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("SPEG_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to SPEG_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("SPEG_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to SPEG_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("SPEG_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to SPEG_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("SPEG_PORT")),
  ),
