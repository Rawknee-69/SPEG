...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "SPEG (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "SPEG (Nightly)");
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                SPEG_DESKTOP_UPDATE_REPOSITORY: "pingdotgg/speg",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "pingdotgg/speg",
              },
            }),
          ),
        ),
      ...
      resolveDesktopRuntimeDependencies(
        {
          ...
          "@speg/contracts": "workspace:*",
          "@speg/shared": "workspace:*",
          "@speg/ssh": "workspace:*",
          "@speg/tailscale": "workspace:*",
          effect: "catalog:",
          electron: "41.5.0",
        },
        {
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      // entry advertises MimeType=x-scheme-handler/speg; for OAuth deep links.
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      ...
      SPEG_MACOS_PROVISIONING_PROFILE: "/tmp/speg.provisionprofile",
      SPEG_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });
    ...
    assert.deepStrictEqual(configuration, {
      ...
      provisioningProfilePath: "/tmp/speg.provisionprofile",
    });
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.speg.speg</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const missingProfileError = captureError({
      SPEG_APPLE_TEAM_ID: "ABC1234567",
      SPEG_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev",
    });
    assert.instanceOf(missingProfileError, MissingMacPasskeyProvisioningProfileError);
    assert.equal(
      missingProfileError.message,
      "SPEG_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      SPEG_APPLE_TEAM_ID: "ABC1234567",
      SPEG_MACOS_PROVISIONING_PROFILE: "/tmp/speg.provisionprofile",
      SPEG_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          SPEG_APPLE_TEAM_ID: "ABC1234567",
          SPEG_MACOS_PROVISIONING_PROFILE: "/tmp/speg.provisionprofile",
          SPEG_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    const invalidPublishableKeyError = captureError({
      SPEG_APPLE_TEAM_ID: "ABC1234567",
      SPEG_MACOS_PROVISIONING_PROFILE: "/tmp/speg.provisionprofile",
      SPEG_CLERK_PUBLISHABLE_KEY: "pk_test_%",
    });
    ...
    assert.equal(invalidPublishableKeyError.message, "SPEG_CLERK_PUBLISHABLE_KEY is invalid.");
  ...
  it("stages the sidecar binaries as external executable resources", () => {
    ...
    assert.equal(resourceMonitorExecutableName("mac"), "speg-resource-monitor");
    assert.equal(resourceMonitorExecutableName("win"), "speg-resource-monitor.exe");
  });
      ...
      const resolved = yield* resolveBuildOptions({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                SPEG_DESKTOP_SKIP_BUILD: "true",
                SPEG_DESKTOP_KEEP_STAGE: "true",
                SPEG_DESKTOP_SIGNED: "true",
                SPEG_DESKTOP_VERBOSE: "true",
                SPEG_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
