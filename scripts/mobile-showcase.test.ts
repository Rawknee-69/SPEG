...
it("selects an explicit CI Android ABI without changing the local default", () => {
  ...
  assert.throws(() => resolveShowcaseAndroidAbi("mips"), /Unsupported SPEG_SHOWCASE_ANDROID_ABI/u);
});
...
it("maps capture scenes to the real application routes", () => {
  assert.equal(showcaseSceneUrl("threads", "environment-1"), "speg://");
  assert.equal(showcaseSceneUrl("environments", "environment-1"), "speg://settings/environments");
  assert.equal(
    showcaseSceneUrl("thread", "environment-1"),
    "speg://threads/environment-1/remote-command-center",
  );
  assert.equal(
    showcaseSceneUrl("terminal", "environment-1"),
    "speg://threads/environment-1/remote-command-center/terminal?terminalId=term-1",
  );
  assert.equal(
    showcaseSceneUrl("review", "environment-1"),
    "speg://threads/environment-1/remote-command-center/review",
  );
});
