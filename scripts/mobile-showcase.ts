...
const ANDROID_PACKAGE = "com.speg.speg";
const APP_SCHEME = "speg";
const IOS_READY_FILENAME = "SpegShowcaseReadyScene";
const SERVER_HOST = "0.0.0.0";
const IOS_SIMULATOR_ARCH = NodeProcess.arch === "arm64" ? "arm64" : "x86_64";
const IOS_APP_PATH = NodePath.join(
  MOBILE_ROOT,
  ".showcase/ios-derived-data/Build/Products/Debug-iphonesimulator/Speg.app",
);
...
Usage:
  pnpm --filter @speg/mobile screenshots [options]
...
async function buildIos(): Promise<string> {
  ...
  await runCommand(
    ...
    [
      "-workspace",
      NodePath.join(MOBILE_ROOT, "ios/Speg.xcworkspace"),
      "-scheme",
      "Speg",
      "-configuration",
      "Debug",
...
async function captureIos(
  ...
  const scenePath = NodePath.join(
    ...
    "Library/Caches/SpegShowcaseScene",
  );
  const readyPath = NodePath.join(
    await iosAppContainer(simulator.udid),
    "Library/Caches",
...
async function writeAndroidShowcaseScene(serial: string, scene: ShowcaseScene): Promise<void> {
  await runAdb(serial, [
    "shell",
    `run-as ${ANDROID_PACKAGE} sh -c 'mkdir -p files && rm -f files/speg-showcase-ready && printf %s ${scene} > files/speg-showcase-scene'`,
  ]);
}
...
async function main(): Promise<void> {
  ...
  const showcaseRootDir = await NodeFSP.mkdtemp(
    NodePath.join(NodeOS.tmpdir(), "speg-mobile-showcase-"),
  );
