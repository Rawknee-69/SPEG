...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.SPEG_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.SPEG_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.SPEG_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.SPEG_RELAY_URL).toBeUndefined();
    expect(env.VITE_SPEG_RELAY_URL).toBeUndefined();
    expect(env.SPEG_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.SPEG_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.SPEG_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.SPEG_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.SPEG_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.SPEG_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "SPEG_CLERK_PUBLISHABLE_KEY=pk_root\nSPEG_CLERK_JWT_TEMPLATE=template_root\nSPEG_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nSPEG_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "SPEG_CLERK_PUBLISHABLE_KEY=pk_local\nSPEG_CLERK_JWT_TEMPLATE=template_local\nSPEG_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nSPEG_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).SPEG_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          SPEG_CLERK_PUBLISHABLE_KEY: "pk_ci",
          SPEG_CLERK_JWT_TEMPLATE: "template_ci",
          SPEG_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          SPEG_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      SPEG_RELAY_URL: "https://ci.example.test",
      VITE_SPEG_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        SPEG_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_SPEG_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          SPEG_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          SPEG_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          SPEG_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          SPEG_RELAY_URL: "https://relay.example.test",
          SPEG_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          SPEG_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          SPEG_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      SPEG_RELAY_URL: "https://relay.example.test",
      VITE_SPEG_RELAY_URL: "https://relay.example.test",
      SPEG_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      SPEG_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      SPEG_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "speg-public-config-"));
