...
> **Revision v6**: jcode is **DROPPED** — provider adapter, session parser, and harness-API storage removed from speg. **No self-built harness for now**: support is limited to the harnesses SPEG already supports (Claude Code, Codex, OpenCode, Cursor, Grok). Historical jcode rows below are marked superseded.
> **No separate web UI** — SPEG IS the UI. Observability dashboard deferred to Phase 6.
...
| 1.11 | **CACM Right Panel Tab** (SPEG web) | ✅ | TS | `apps/web/src/components/speg/CacmPanel.tsx` | 1.9 |
| 1.12 | **SPEG Settings Panel** (SPEG web) | ✅ | TS | `apps/web/src/components/speg/SpegSettings.tsx` | — |
...
| v5 | v6 | Why |
|----|-----|-----|
| speg-web/ (React app) | **SPEG right panel tabs** | SPEG IS the UI. No redundant app. |
| speg-desktop/ (Electron) | **SPEG desktop** | Already exists. The already-supported harnesses bundle into it. |
| Standalone SPEG web UI | **Existing provider adapters** | Claude Code, Codex, OpenCode, Cursor, Grok — SPEG's existing adapter system |
| CACM timeline component | **CACM right panel tab** | Same data, rendered in SPEG's side panel |
| Provider picker component | **SPEG model picker** | Already exists in SPEG's composer |

### SPEG Surfaces We Extend
...
1. Open SPEG → composer shows the already-supported providers (Claude, Codex, OpenCode, Cursor, Grok)
