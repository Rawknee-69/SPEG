...
Define the long-term server auth architecture for SPEG before first-class remote environments ship.
...
Every privileged surface of the SPEG server must go through the same auth policy engine:
...
Participants:
  ...
  SpegServer      = local backend child process
...
DesktopMain -> SpegServer : spawn server (--bootstrap-fd ...)
DesktopMain -> SpegServer : send desktop bootstrap envelope
note over SpegServer : policy = DesktopManagedLocalPolicy
note over SpegServer : allowed pairing = desktop-bootstrap only

Frontend -> DesktopMain : request local bootstrap grant
DesktopMain --> Frontend : short-lived desktop bootstrap grant

Frontend -> SpegServer : POST /api/auth/bootstrap
SpegServer -> SpegServer : validate desktop bootstrap grant
SpegServer -> SpegServer : create browser session
SpegServer --> Frontend : Set-Cookie: session=...

Frontend -> SpegServer : GET /ws + authenticated cookie
SpegServer -> SpegServer : validate cookie session
SpegServer --> Frontend : websocket accepted
```
...
```text
Participants:
  UserShell     = npx speg launcher
  SpegServer      = standalone local server
  Browser       = browser tab

UserShell -> SpegServer : start server
SpegServer -> SpegServer : getOrCreate("server-signing-key")
note over SpegServer : policy = LoopbackBrowserPolicy

UserShell -> SpegServer : issue one-time pairing token
SpegServer --> UserShell : pairing URL or pairing token

UserShell --> Browser : open /pair?token=...

Browser -> SpegServer : GET /pair?token=...
SpegServer -> SpegServer : validate one-time token
SpegServer -> SpegServer : create browser session
SpegServer --> Browser : Set-Cookie: session=...
SpegServer --> Browser : redirect to app

Browser -> SpegServer : GET /ws + authenticated cookie
SpegServer --> Browser : websocket accepted
```
...
Participants:
  ...
  SpegServer      = SPEG server
  PhoneBrowser  = mobile browser

DesktopUser -> DesktopMain : enable remote access via tunnel
DesktopMain -> SpegServer : switch policy to RemoteReachablePolicy
DesktopMain -> Tunnel : publish local SPEG endpoint
Tunnel --> DesktopMain : public https/wss URL

DesktopMain -> SpegServer : issue one-time pairing token
SpegServer --> DesktopMain : pairing token
...
Tunnel -> SpegServer : forward request
SpegServer -> SpegServer : validate one-time token
SpegServer -> SpegServer : create mobile browser session
SpegServer --> PhoneBrowser : Set-Cookie: session=...
SpegServer --> PhoneBrowser : redirect to app

PhoneBrowser -> Tunnel : GET /ws + authenticated cookie
Tunnel -> SpegServer : forward websocket upgrade
SpegServer --> PhoneBrowser : websocket accepted
```
...
DesktopUser -> SpegServer : enable private-network access
SpegServer -> SpegServer : switch policy to RemoteReachablePolicy
DesktopUser -> SpegServer : issue one-time pairing token
SpegServer --> DesktopUser : pairing URL / QR

DesktopUser -> PhoneBrowser : open private-network URL
PhoneBrowser -> PrivateNet : GET http(s)://private-host/pair?token=...
PrivateNet -> SpegServer : route request
SpegServer -> SpegServer : validate one-time token
SpegServer -> SpegServer : create mobile browser session
SpegServer --> PhoneBrowser : Set-Cookie: session=...
SpegServer --> PhoneBrowser : redirect to app
...
PrivateNet -> SpegServer : websocket upgrade
SpegServer --> PhoneBrowser : websocket accepted
```
...
Participants:
  ...
  RemoteSpeg      = remote SPEG server
  Frontend      = desktop renderer

DesktopUser -> DesktopMain : add SSH host
DesktopMain -> SSH : connect to remote host
SSH -> RemoteHost : probe environment / verify speg availability
DesktopMain -> SSH : run remote launch command
SSH -> RemoteHost : speg remote launch --json
RemoteHost -> RemoteSpeg : start or reuse server
RemoteSpeg --> RemoteHost : port + environment metadata
...
note over RemoteSpeg : policy = RemoteReachablePolicy
note over DesktopMain,RemoteSpeg : desktop may use a trusted bootstrap flow here

Frontend -> DesktopMain : request bootstrap for selected environment
DesktopMain --> Frontend : short-lived bootstrap grant

Frontend -> RemoteSpeg : POST /api/auth/bootstrap via forwarded port
RemoteSpeg -> RemoteSpeg : validate bootstrap grant
RemoteSpeg -> RemoteSpeg : create browser session
RemoteSpeg --> Frontend : Set-Cookie: session=...

Frontend -> RemoteSpeg : GET /ws + authenticated cookie
RemoteSpeg --> Frontend : websocket accepted
```
