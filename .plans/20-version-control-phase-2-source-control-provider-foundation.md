...
- review threads/comments/diffs: key by provider CR ID and head SHA/version when available; fetch on demand for SPEG Review
...
- The provider API includes the review operations needed by future SPEG Review work, even if they are capability-gated.
