...
const remoteDebuggingPort = process.env.SPEG_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line speg/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.SPEG_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.SPEG_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--speg-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--speg-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
