// This file mostly exists because we want dev mode to say "SPEG (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "SPEG (Dev)" : "SPEG (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.speg.speg.dev.${devBundleIdSuffix || "local"}`
  : "com.speg.speg";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["speg-dev"] : ["speg"];
...
// oxlint-disable-next-line speg/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["SPEG_PORT", environment.SPEG_PORT],
    ["SPEG_HOME", environment.SPEG_HOME],
    ["SPEG_COMMIT_HASH", environment.SPEG_COMMIT_HASH],
    ["SPEG_OTLP_TRACES_URL", environment.SPEG_OTLP_TRACES_URL],
    ["SPEG_OTLP_EXPORT_INTERVAL_MS", environment.SPEG_OTLP_EXPORT_INTERVAL_MS],
    ["SPEG_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --speg-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the SPEG name.
