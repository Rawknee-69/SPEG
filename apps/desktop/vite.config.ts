...
const shouldLaunchElectronAfterPack = process.env.SPEG_DESKTOP_DEV === "1";
  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["speg#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-preview-annotation-css.mjs && cross-env SPEG_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["speg#build"],
        cache: false,
      },
      "dev:bundle": {
  ...
  pack: [
    {
      ...
      clean: true,
      deps: {
        alwaysBundle: (id) => id.startsWith("@speg/"),
      },
