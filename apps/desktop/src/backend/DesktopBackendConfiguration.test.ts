    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          SPEG_PORT: "9999",
          SPEG_MODE: "desktop",
          SPEG_DESKTOP_LAN_HOST: "192.168.1.50",
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "speg-desktop-backend-config-test-",
    });
...
describe("DesktopBackendConfiguration", () => {
  it.effect("resolvePrimary produces a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.SPEG_PORT);
        assert.isUndefined(first.env.SPEG_MODE);
        assert.isUndefined(first.env.SPEG_DESKTOP_LAN_HOST);
        ...
        assert.equal(first.bootstrap.spegHome, environment.baseDir);
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const linuxEntryPath = "/tmp/speg code's launch/entry file.mjs";
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.equal(config.bootstrap.spegHome, environment.baseDir);
          assert.isTrue(Option.isNone(config.preflightFailure));
        }).pipe(
          Effect.provide(
            DesktopBackendConfiguration.layer.pipe(
  ...
  it.effect("prefers the external packaged resource monitor over the copy inside the asar", () =>
    Effect.gen(function* () {
      ...
      const embeddedMonitorPath = `${resourcesPath}/app.asar/apps/desktop/prod-resources/resource-monitor/speg-resource-monitor`;
      const monitorPath = `${resourcesPath}/resource-monitor/speg-resource-monitor`;
  ...
  it.effect("prefers the release resource monitor when both development builds exist", () =>
    Effect.gen(function* () {
      ...
      const releaseMonitorPath = path.join(
        baseDir,
        "native/resource-monitor/target/release/speg-resource-monitor",
      );
      const debugMonitorPath = path.join(
        baseDir,
        "native/resource-monitor/target/debug/speg-resource-monitor",
      );
  ...
  it("resolvePrimaryLabel is runSync-safe against the real WSL availability probe", async () => {
    ...
    // oxlint-disable-next-line speg/no-manual-effect-runtime-in-tests -- This test intentionally replicates the sync IPC handler's runSync path to catch a regression to async-only resolution; it.effect would mask it.
    const runtime = ManagedRuntime.make(
      DesktopBackendConfiguration.layer.pipe(
        ...
        Layer.provideMerge(makeEnvironmentLayer("/tmp/speg-wsl-isavailable", { platform: "win32" })),
    ...
    try {
      ...
      // oxlint-disable-next-line speg/no-manual-effect-runtime-in-tests -- Same reason: this is the synchronous resolution the IPC handler performs.
