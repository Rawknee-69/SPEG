...
const makeDesktopClerkLayer = (isDevelopment = true, events: string[] = []) => {
  const environment = DesktopEnvironment.DesktopEnvironment.of({
    stateDir: "/tmp/speg-state",
    isDevelopment,
    appDataDirectory: "/tmp/app-data",
    userDataDirName: isDevelopment ? "speg-dev" : "speg",
    legacyUserDataDirName: isDevelopment ? "SPEG (Dev)" : "SPEG (Alpha)",
...
describe("DesktopClerk", () => {
  ...
  it.effect("acquires and releases the SDK bridge with the layer", () => {
    ...
    return Effect.gen(function* () {
        ...
        [
          {
            ...
            passkeys: true,
            renderer: { scheme: "speg-dev", host: "app" },
          },
        ],
      ]);
      ...
      assert.deepEqual(events, ["setPath:userData:/tmp/app-data/speg-dev", "createClerkBridge"]);
  ...
  it.effect("preserves bridge initialization failures", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(error.stateDir, "/tmp/speg-state");
      assert.equal(error.isDevelopment, true);
      assert.strictEqual(error.cause, cause);
      assert.equal(
        error.message,
        'Failed to initialize the desktop Clerk bridge for state directory "/tmp/speg-state" (development: true).',
      );
    });
  });
  ...
  it.effect("preserves bridge cleanup failures", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(
          error.message,
          'Failed to clean up the desktop Clerk bridge for state directory "/tmp/speg-state" (development: false).',
        );
      }
    });
  });
  ...
  it.each([
    { isDevelopment: true, scheme: "speg-dev" },
    { isDevelopment: false, scheme: "speg" },
    ...
    assert.equal(DesktopClerk.createDesktopClerkBridge("/tmp/speg-state", isDevelopment), bridge);
    assert.deepEqual(storageMock.mock.calls, [[{ path: "/tmp/speg-state" }]]);
    assert.deepEqual(createClerkBridgeMock.mock.calls, [
      [
        {
          storage: storageAdapter,
