...
const AppPackageMetadata = Schema.Struct({
  spegCommitHash: Schema.optional(Schema.String),
});
...
>()("@speg/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.spegCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
