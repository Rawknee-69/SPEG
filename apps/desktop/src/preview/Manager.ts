...
const makeNativeOperations = Effect.fn("PreviewManager.makeOperations")(function* (
  ...
  const ensurePlaywrightInjected = Effect.fn("PreviewManager.ensurePlaywrightInjected")(function* (
    ...
    const installed = yield* evaluateWithDebugger<boolean>(
      tabId,
      send,
      "Boolean(globalThis.__spegPlaywrightInjected)",
      true,
    );
    if (installed) return;
  ...
  const resolveClickPoint = Effect.fn("PreviewManager.resolveClickPoint")(function* (
    ...
    >(
      ...
      `(() => {
          try {
            const injected = globalThis.__spegPlaywrightInjected;
  ...
  const typeIntoAutomationTarget = Effect.fn("PreviewManager.typeIntoAutomationTarget")(function* (
    ...
    >(
      ...
      `(() => {
          try {
            const element = ${locatorJson ? `(() => { const injected = globalThis.__spegPlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, true); })()` : "document.activeElement"};
            if (!element) return { notFound: true };
            const textControl =
  ...
  const performAutomationScroll = Effect.fn("PreviewManager.performAutomationScroll")(function* (
    ...
    >(
      ...
      `(() => {
        try {
          const target = ${locatorJson ? `(() => { const injected = globalThis.__spegPlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, true); })()` : "window"};
  ...
  const performAutomationWaitFor = Effect.fn("PreviewManager.performAutomationWaitFor")(function* (
    ...
    while ((yield* currentMillis) <= deadline) {
      ...
      >(
        ...
        `(() => {
              try {
                const selectorMatched = ${locatorJson ? `(() => { const injected = globalThis.__spegPlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, false) !== null; })()` : "true"};
                const textMatched = ${
...
>()("@speg/desktop/preview/Manager/PreviewManager") {}
