...
const OVERLAY_ATTRIBUTE = "data-speg-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--speg-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--speg-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--speg-radius": theme.radius,
    "--speg-background": theme.background,
    "--speg-foreground": theme.foreground,
    "--speg-popover": theme.popover,
    "--speg-popover-foreground": theme.popoverForeground,
    "--speg-primary": theme.primary,
    "--speg-primary-foreground": theme.primaryForeground,
    "--speg-muted": theme.muted,
    "--speg-muted-foreground": theme.mutedForeground,
    "--speg-accent": theme.accent,
    "--speg-accent-foreground": theme.accentForeground,
    "--speg-border": theme.border,
    "--speg-input": theme.input,
    "--speg-ring": theme.ring,
    "--speg-font-sans": theme.fontSans,
    "--speg-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-speg-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--speg-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-speg-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
