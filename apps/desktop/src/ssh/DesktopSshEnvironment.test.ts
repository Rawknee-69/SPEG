...
import { SshPasswordPromptError } from "@speg/ssh/errors";
...
function makeTempHomeDir() {
  return Effect.gen(function* () {
    ...
    return yield* fs.makeTempDirectoryScoped({ prefix: "speg-ssh-env-test-" });
  });
}
...
describe("sshEnvironment", () => {
  it("keeps prompt presentation diagnostics distinct from the legacy wrapper message", () => {
    ...
    assert.equal(
      ...
      "SPEG window is not available for SSH authentication.",
    );
  });
