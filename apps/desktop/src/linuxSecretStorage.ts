...
function getGnomeKeyringRemediationMessage(): string {
  return "SPEG could not access GNOME Keyring to save this environment credential. Install and start GNOME Keyring, then restart SPEG.";
}

function getKWalletRemediationMessage(): string {
  return "SPEG could not access KWallet to save this environment credential. Enable the KDE wallet subsystem in System Settings, then restart SPEG.";
}
