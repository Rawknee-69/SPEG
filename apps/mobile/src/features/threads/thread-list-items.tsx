...
import type { EnvironmentThreadSearchMatch } from "@speg/client-runtime/state/thread-search";
    ...
    >
      ...
      >
        ...
        <Text
          className={
            compact
              ? "flex-shrink text-base font-speg-bold tracking-[0.2px] text-foreground-muted"
              : "flex-shrink text-sm font-speg-bold tracking-[0.2px] text-foreground-muted"
          }
          numberOfLines={1}
        >
          {props.title}
        ...
        <Text
          className={
            compact
              ? "flex-1 text-sm font-speg-medium text-foreground-tertiary"
              : "flex-1 text-xs font-speg-medium text-foreground-tertiary"
          }
        >
          {props.threadCount}
        </Text>
    ...
    >
      <View className="flex-row items-center gap-1.5">
        ...
        <Text
          className={
            compact
              ? "text-sm font-speg-medium text-foreground-muted"
              : "text-xs font-speg-medium text-foreground-muted"
          }
        >
          {label}
        </Text>
...
export const PendingTaskListRow = memo(function PendingTaskListRow(props: {
  ...
  const statusPill = (
    <View className="rounded-full bg-zinc-500/12 px-1.5 py-0.5 dark:bg-zinc-500/16">
      <Text className="text-3xs font-speg-bold text-zinc-600 dark:text-zinc-300">Pending</Text>
    </View>
  );
  ...
  const rowContent = compact ? (
      ...
      >
        ...
        >
          <View className="flex-row items-center justify-between gap-2">
            <Text className="flex-1 text-lg font-speg-bold text-foreground" numberOfLines={1}>
              {pendingTask.title}
            </Text>
    ...
    >
      <View className="gap-[3px]">
        <View className="flex-row items-center justify-between gap-2">
          <Text className="flex-1 text-base font-speg-medium text-foreground" numberOfLines={1}>
            {pendingTask.title}
          </Text>
          <View className="flex-row items-center gap-2">
            {statusPill}
...
export const ThreadListRow = memo(function ThreadListRow(props: {
  ...
  const statusPill = effectiveStatus ? (
    <View className={`${effectiveStatus.pillClassName} rounded-full px-1.5 py-0.5`}>
      <Text className={`text-3xs font-speg-bold ${effectiveStatus.textClassName}`}>
      ...
      <View className="mt-px flex-row items-center gap-1.5">
        ...
        {pr !== null ? (
          <View className="flex-row items-center gap-0.5">
            ...
            />
            <Text
              className={`${compact ? "text-sm" : "text-xs"} font-speg-medium ${
      ...
      >
        <View className="gap-[3px]">
          <View className="flex-row items-center justify-between gap-2">
            <Text
              className={cn(
                "flex-1 text-base font-speg-medium",
