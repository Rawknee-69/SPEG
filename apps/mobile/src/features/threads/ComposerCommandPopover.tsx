...
import type { ComposerTriggerKind } from "@speg/shared/composerTrigger";
import type { ServerProviderSkill, ServerProviderSlashCommand } from "@speg/contracts";
...
const CommandRow = memo(function CommandRow(props: {
  ...
  return (
    ...
    >
      ...
      <Text className="shrink-0 text-base font-speg-medium text-foreground" numberOfLines={1}>
        {props.item.label}
      </Text>
    ...
    <PopoverSurface isDarkMode={isDarkMode}>
      {label ? (
        <View className="px-3.5 pt-2.5 pb-1">
          <Text className="text-3xs font-speg-bold tracking-[0.8px] uppercase text-foreground-muted">
            {label}
          </Text>
        </View>
      ) : null}
