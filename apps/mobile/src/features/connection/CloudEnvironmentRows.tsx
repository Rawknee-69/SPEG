...
interface CloudEnvironmentRowsProps {
  ...
  /**
   * Hide the "SPEG Connect" section title + refresh button for hosts that
...
/**
 * "SPEG Connect" section: every environment published to the signed-in account,
 * with connect switches, availability status, refresh, and loading/error
 * states. Shared between the Settings environments screen and the SPEG Connect
 * onboarding sheet.
 *
    ...
    <View collapsable={false} className={cn("gap-3", showHeader && "mt-5")}>
      {showHeader ? (
        <View className="flex-row items-center justify-between px-1">
          <Text className="text-sm font-speg-bold uppercase text-foreground-muted">SPEG Connect</Text>
          {discoveryAvailable ? (
            <Pressable
      ...
      !controller.relayDiscovery.isRefreshing ? (
        <View collapsable={false} className="gap-3 rounded-[24px] bg-card p-5">
          <Text className="text-base font-speg-bold text-foreground">
            Could not load SPEG Connect environments
          </Text>
          ...
          >
            <Text className="text-xs font-speg-bold text-foreground">Try again</Text>
          </Pressable>
        </View>
      ) : null}
    </View>
    ...
    >
      <View className="min-w-0 flex-1 gap-0.5">
        <View className="min-w-0 flex-row items-center gap-2">
          ...
          <Text
            className="min-w-0 flex-shrink text-base font-speg-bold leading-snug text-foreground"
            numberOfLines={1}
          >
            {props.label}
          </Text>
...
function CopyTraceIdButton(props: { readonly traceId: string }) {
  ...
  return (
    ...
    >
      ...
      <Text className="text-xs font-speg-bold text-foreground">Copy trace ID</Text>
    </Pressable>
  );
}
