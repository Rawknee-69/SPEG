...
public final class SpegTerminalView: ExpoView, UITextFieldDelegate {
  ...
  var terminalKey: String = "" {
    didSet {
      accessibilityIdentifier = "speg-terminal-\(terminalKey)"
  ...
  private func setupWriteCallback() {
    ...
    ghostty_surface_set_write_callback(surface, { userdata, data, len in
      ...
      let view = Unmanaged<SpegTerminalView>.fromOpaque(userdata).takeUnretainedValue()
  ...
  private func writeThemeConfigFile() -> String? {
    ...
    let url = URL(fileURLWithPath: NSTemporaryDirectory())
      .appendingPathComponent("speg-terminal-theme-\(appearance.rawValue).ghostty")
