...
#import "SpegMarkdownTextShadowNode.h"
#import "SpegMarkdownTextComponentDescriptor.h"
...
static void SpegMarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<SpegMarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void SpegMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<SpegMarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = SpegMarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        SpegMarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
...
@protocol SpegMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface SpegMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<SpegMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<SpegMarkdownOutsideTapTarget>)target;

@end

static const void *SpegMarkdownOutsideTapCoordinatorKey =
    &SpegMarkdownOutsideTapCoordinatorKey;

@implementation SpegMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<SpegMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<SpegMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, SpegMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<SpegMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static SpegMarkdownOutsideTapCoordinator *
SpegMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  SpegMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, SpegMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[SpegMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        SpegMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface SpegMarkdownText () <RCTSpegMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface SpegMarkdownText () <SpegMarkdownOutsideTapTarget>
@end

@implementation SpegMarkdownText {
  UIView * _view;
  UITextView * _textView;
  SpegMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<SpegMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const SpegMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    SpegMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, SpegMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [SpegMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<SpegMarkdownTextProps const>(_props);
  ...
  SpegMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  SpegMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::SpegMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::SpegMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<SpegMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<SpegMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<SpegMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == SpegMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == SpegMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == SpegMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == SpegMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const SpegMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (SpegMarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[SpegMarkdownTextRun class]]) {
      continue;
    }

    SpegMarkdownTextRun* textChild = (SpegMarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::SpegMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::SpegMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> SpegMarkdownTextCls(void)
{
  return SpegMarkdownText.class;
}
