...
import SpegMarkdownTextRunNativeComponent from "./SpegMarkdownTextRunNativeComponent";
import SpegMarkdownTextNativeComponent from "./SpegMarkdownTextNativeComponent";
...
function MarkdownTextPrimitiveChild({ style, children, ...rest }: MarkdownTextPrimitiveProps) {
  ...
  const nativeChildren = React.Children.toArray(children).map((child) => {
    ...
    return (
      ...
      <SpegMarkdownTextRunNativeComponent
  ...
  if (!isAncestor) {
    return (
      <TextAncestorContext.Provider value={contextValue}>
        <SpegMarkdownTextNativeComponent
          {...textDefaults}
          {...rest}
