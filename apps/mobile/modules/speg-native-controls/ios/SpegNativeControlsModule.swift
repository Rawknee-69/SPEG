...
public final class SpegNativeControlsModule: Module {
  public func definition() -> ModuleDefinition {
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/SpegShowcaseScene"
    ...
    AsyncFunction("applyShowcaseOrientation") { (orientation: String) in
      ...
      for case let windowScene as UIWindowScene in UIApplication.shared.connectedScenes {
        windowScene.requestGeometryUpdate(.iOS(interfaceOrientations: mask)) { error in
          NSLog("SpegNativeControls applyShowcaseOrientation(\(orientation)) failed: \(error)")
        }
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/SpegShowcaseReadyScene"
      try? scene.write(toFile: readyPath, atomically: true, encoding: .utf8)
    }

    View(SpegHeaderButtonView.self) {
      Prop("label") { (view: SpegHeaderButtonView, label: String) in
        view.setLabel(label)
      }
      Prop("systemImage") { (view: SpegHeaderButtonView, systemImage: String) in
