...
public final class SpegKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("SpegKeyboardCommands")

    View(SpegKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: SpegKeyboardCommandsView, commands: [String]) in
...
public final class SpegKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.spegFirstResponder == nil else { return }
...
private extension UIView {
  var spegFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.spegFirstResponder { return responder }
    }
    return nil
  }
}
