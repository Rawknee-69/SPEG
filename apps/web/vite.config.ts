...
import { DEV_PROXIED_PATH_PREFIXES } from "@speg/shared/devProxy";
...
const isSingleOriginDev = process.env.SPEG_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_SPEG_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.SPEG_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: SPEG_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); SPEG_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.SPEG_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // SPEG_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.SPEG_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.SPEG_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_SPEG_RELAY_URL": JSON.stringify(configuredRelayUrl),
