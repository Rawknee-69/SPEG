...
const HOSTED_WEB_CHANNEL_COOKIE = "speg_web_channel";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @speg/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --ignore-scripts --filter '@speg/scripts...' --filter '@speg/web...'",
  routes: [
    {
      src: "/__speg/channel",
