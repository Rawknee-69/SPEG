...
/**
 * Drives the primary environment's SPEG Connect link. SPEG Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message = cause instanceof Error ? cause.message : "Could not update SPEG Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[speg-connect] Could not update SPEG Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update SPEG Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn SPEG Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to SPEG Connect before enabling this."));
        return false;
      }
