...
import { EnvironmentRegistry } from "@speg/client-runtime/connection";
import { request, runStream } from "@speg/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@speg/client-runtime/rpc";
import { ManagedRelay } from "@speg/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `SPEG cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `SPEG cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "SPEG_RELAY_URL is not configured.",
      });
    }
...
// independently of SPEG Connect while clients reach the environment out of band.
