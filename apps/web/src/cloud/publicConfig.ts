import { relayClerkTokenOptions } from "@speg/shared/relayAuth";
...
export class CloudPublicConfigMissingError extends Schema.TaggedErrorClass<CloudPublicConfigMissingError>()(
  ...
  {
    key: Schema.Literal("SPEG_CLERK_JWT_TEMPLATE"),
  },
) {
...
export function resolveCloudPublicConfig(): CloudPublicConfig {
  return {
    ...
    relayUrl: normalizeSecureRelayUrl(
      (import.meta.env.VITE_SPEG_RELAY_URL as string | undefined) ?? "",
    ),
    relayTracing: {
...
export function resolveRelayClerkTokenOptions() {
  ...
  if (!clerkJwtTemplate) {
    throw new CloudPublicConfigMissingError({ key: "SPEG_CLERK_JWT_TEMPLATE" });
  }
