...
describe("theme failure handling", () => {
  ...
  it("reads the persisted SPEG Chat theme preference", async () => {
    vi.stubGlobal("window", {
      localStorage: createStorage({
        getItem: () => "speg-chat",
      }),
    ...
    expect(readThemePreference()).toBe("speg-chat");
  });
  ...
  it("falls back during initial theme application and logs only safe attributes", async () => {
    ...
    expect(errorLog).toHaveBeenCalledWith(
      "Failed to read theme preference for speg:theme.",
  ...
  it("retries a failed storage read only after a relevant storage event", async () => {
    ...
    const getItem = vi.fn((key: string) => (key === "speg:theme" ? themeGetItem() : null));
    const errorLog = vi.spyOn(console, "error").mockImplementation(() => {});
    ...
    storageHandler?.({ key: "speg:theme" } as StorageEvent);
    readSnapshot?.();
