import { findErrorTraceId } from "@speg/client-runtime/errors";
import {
...
import type { RelayClientEnvironmentRecord } from "@speg/contracts/relay";
...
/**
 * The user's SPEG Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through SPEG Connect.`,
      });
      return;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the SPEG Connect environment.";
    const traceId = findErrorTraceId(cause);
    console.error("[speg-connect] Could not connect environment", { message, traceId, cause });
    toastManager.add({
      type: "error",
