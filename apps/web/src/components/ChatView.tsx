...
import { requestOlderThreadTurns, threadHasOlderTurns } from "@speg/client-runtime/state/threads";
...
function ChatViewContent(props: ChatViewProps) {
  ...
  const addCacmSurface = useCallback(() => {
    if (!activeThreadRef) return;
    useRightPanelStore.getState().open(activeThreadRef, "cacm");
  }, [activeThreadRef]);
  const injectCacmContext = useCallback(
    (formatted: string) => {
      composerRef.current?.insertTextAtEnd(formatted, { ensureLeadingBoundary: true });
    },
    [composerRef],
  );
  ...
  // Collapsing that space during a gesture clamps the viewport back to the end.  // Manual navigation stops live-follow without removing anchored end space.
    ...
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      >
        ...
        <header
          ...
          data-glass-light=""
          className={cn(
            "glass-topbar transition-[padding-left] duration-200 ease-linear motion-reduce:transition-none",
            isElectron
              ? cn(
              ...
              >
                <div className="pointer-events-auto relative z-10">
                  ...
                  >
                    ...
                    >
                      <div
                        className="chat-composer-glass-host relative z-10 w-full rounded-[22px]"
                        data-glass-light=""
                      >
