...
export function ResourceTelemetryDiagnostics() {
  ...
  const allSpeg = snapshot?.groups.allSpeg;
        ...
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03),0_8px_30px_rgb(0_0_0/0.035)]">
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-border/55 md:grid-cols-3">
            <IconStat
              ...
              label="Current CPU"
              value={allSpeg ? `${allSpeg.currentCpuPercent.toFixed(1)}%` : "..."}
              detail={allSpeg ? `${formatCpuTime(allSpeg.cpuTimeMs)} observed CPU time` : undefined}
            />
            <IconStat
              icon={<MemoryStickIcon className="size-3.5" />}
              label="Resident memory"
              value={allSpeg ? formatBytes(allSpeg.currentRssBytes) : "..."}
              detail={
                allSpeg ? `${formatBytes(allSpeg.peakRssBytes)} combined process peaks` : undefined
              }
            />
            <IconStat
              icon={<ActivityIcon className="size-3.5" />}
              label="Process count"
              value={allSpeg ? String(allSpeg.processCount) : "..."}
              detail={
                allSpeg ? `${allSpeg.processStarts} starts · ${allSpeg.processExits} exits` : undefined
              }
            />
            <IconStat
              icon={<HardDriveIcon className="size-3.5" />}
              label="Read throughput"
              value={allSpeg ? formatRate(allSpeg.ioReadBytesPerSecond) : "..."}
              detail={allSpeg ? `${formatBytes(allSpeg.ioReadBytes)} observed` : undefined}
            />
            <IconStat
              icon={<DatabaseIcon className="size-3.5" />}
              label="Write throughput"
              value={allSpeg ? formatRate(allSpeg.ioWriteBytesPerSecond) : "..."}
              detail={allSpeg ? `${formatBytes(allSpeg.ioWriteBytes)} observed` : undefined}
              tone={
                allSpeg && allSpeg.ioWriteBytesPerSecond >= 10 * 1_024 * 1_024
                  ? "danger"
                  : allSpeg && allSpeg.ioWriteBytesPerSecond >= 1_024 * 1_024
                    ? "warning"
                    : "default"
              }
            />
      ...
      >
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03)]">
          <div className="bg-muted/15 px-4 py-3 text-[11px] leading-relaxed text-muted-foreground sm:px-5">
            ...
            counters identify known SPEG operations so process spikes can be correlated with specific
