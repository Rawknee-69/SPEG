...
import { connectionStatusText } from "@speg/client-runtime/connection";
import {
...
function SavedBackendListRow({
  ...
  const metadataBits = [
    ...
    environment.relayManaged ? "SPEG Connect" : null,
  ].filter((value): value is string => value !== null);
...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const disabledReason = !isSignedIn
    ? "Sign in to SPEG Connect to manage this environment."
    : !canManageRelay
      ? "Your session does not have permission to manage SPEG Connect access."
      : null;
    ...
    if (ok) {
      ...
      toastManager.add({
        ...
        title: enabled
          ? "SPEG Connect linked"
          : publishAgentActivity
            ? "SPEG Connect tunnel disabled"
            : "SPEG Connect unlinked",
        description: enabled
          ? "This environment is available through SPEG Connect."
          : publishAgentActivity
            ? "The managed tunnel was removed. Agent activity publishing stays on."
            : "This environment is no longer available through SPEG Connect.",
      });
    }
    setIsUpdating(false);
  };
  ...
  return (
    <>
      <SettingsRow
        ...
        description={
          managedTunnelActive
            ? "This environment is available to your other devices through SPEG Connect."
            : "Make this environment available to your other devices through SPEG Connect."
        }
      ...
      <SettingsRow
        ...
        description="Send activity from this environment to your mobile clients for push notifications and Live Activities. Works without a SPEG Connect tunnel."
        control={
          <CloudLinkSwitch
  ...
  return (
    <Empty className="min-h-52">
      ...
      <EmptyHeader>
        ...
        <EmptyDescription>
          {cloudEnabled
            ? "Click “Add environment” to pair another environment, or connect one from SPEG Connect."
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description="Stop the Windows backend and run only the WSL backend. Useful if you develop entirely inside WSL and don't want a second backend process. SPEG restarts when you change this."
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? "SPEG will restart to expose this environment over the network."
                    : "SPEG will restart and limit this environment back to this machine."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? "SPEG will restart on the Windows backend. Threads and projects opened against WSL stay safe inside the distro and become available again when you re-enable WSL."
                      : "The WSL backend will stop. Threads and projects opened against WSL stay safe inside the distro, but they'll be unavailable in SPEG until you re-enable WSL."
                    : pendingWslChange?.kind === "distro"
                      ? "SPEG will restart the WSL backend on the new distro. Sessions still running on the current distro will be interrupted."
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? "SPEG will restart and start only the WSL backend. Your Windows-side projects won't be accessible until you turn this off again."
                          : "SPEG will restart and bring the Windows backend back up alongside WSL."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  SPEG will restart the local backend without Tailscale Serve.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  SPEG will restart the local backend with Tailscale Serve enabled and ask
