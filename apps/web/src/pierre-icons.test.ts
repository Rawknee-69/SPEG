...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with SPEG-specific exact filename icons", () => {
    assert.equal(
      resolvePierreIconForEntry("package.json", "file")?.name,
      "speg-file-icon-package-json",
    );
    assert.equal(
      resolvePierreIconForEntry("config/tsconfig.json", "file")?.name,
      "speg-file-icon-tsconfig",
    );
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "speg-file-icon-agents");
    assert.equal(resolvePierreIconForEntry("CLAUDE.md", "file")?.name, "speg-file-icon-claude");
    assert.equal(resolvePierreIconForEntry("README.md", "file")?.name, "speg-file-icon-readme");
    assert.equal(resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name, "speg-file-icon-pnpm");
    assert.equal(
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(Object.values(SPEG_PIERRE_ICONS.byFileName));
    for (const iconName of customIconNames) {
      assert.include(SPEG_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
