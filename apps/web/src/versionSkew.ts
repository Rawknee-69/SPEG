import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@speg/contracts";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "speg:version-mismatch-dismissals:v1";
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same SPEG version.",
  };
}
...
export function serverUpdateGuidance(
  ...
  switch (capability) {
    ...
    case "desktop-managed":
      return `The ${serverLabel} is run by the SPEG desktop app on its machine — update the desktop app there to sync them.`;
    default:
