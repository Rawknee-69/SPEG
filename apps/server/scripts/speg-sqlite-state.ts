...
// @effect-diagnostics nodeBuiltinImport:off - node:os resolves the shared SPEG home guard.
import * as NodeRuntime from "@effect/platform-node/NodeRuntime";
import * as NodeServices from "@effect/platform-node/NodeServices";
import * as NodeOS from "node:os";
import { fromJsonStringPretty } from "@speg/shared/schemaJson";
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start SPEG once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedErrorClass<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.speg database. Use an isolated --base-dir.";
  }
}
...
export const runSqliteState = Effect.fn("runSqliteState")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".speg"));
...
export const spegSqliteStateCommand = Command.make(
  "speg-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit SPEG base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated SPEG SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(spegSqliteStateCommand, { version: "0.0.0" }).pipe(
