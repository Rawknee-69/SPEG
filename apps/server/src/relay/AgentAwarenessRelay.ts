...
import { projectThreadAwareness } from "@speg/shared/agentAwareness";
import { makeDrainableWorker } from "@speg/shared/DrainableWorker";
...
>()("speg/relay/AgentAwarenessRelay") {}
...
const makePublishProof = Effect.fn("makePublishProof")(function* (input: {
  ...
  const payload = {
    iss: `speg-env:${input.environmentId}`,
      ...
      switch (startupState) {
        case "waiting-for-link":
          yield* Effect.logInfo(
            "agent activity publishing standby; waiting for SPEG Connect link reconciliation",
          );
          break;
        case "disabled":
          yield* Effect.logInfo("agent activity publishing disabled by SPEG Connect configuration");
          break;
        case "enabled":
