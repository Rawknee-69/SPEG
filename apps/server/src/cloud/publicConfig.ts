import { CONNECT_OAUTH_SCOPES, DEFAULT_HOSTED_APP_URL } from "@speg/shared/connectAuth";
import { clerkFrontendApiUrlFromPublishableKey } from "@speg/shared/relayAuth";
import { normalizeSecureRelayUrl } from "@speg/shared/relayUrl";
...
declare const __SPEG_BUILD_RELAY_URL__: string | undefined;
declare const __SPEG_BUILD_CLERK_PUBLISHABLE_KEY__: string | undefined;
declare const __SPEG_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: string | undefined;
declare const __SPEG_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: string | undefined;
declare const __SPEG_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: string | undefined;
declare const __SPEG_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: string | undefined;
...
export const buildTimeRelayUrl =
  typeof __SPEG_BUILD_RELAY_URL__ === "undefined"
    ? ""
    : (normalizeSecureRelayUrl(__SPEG_BUILD_RELAY_URL__) ?? "");
export const buildTimeClerkPublishableKey = readBuildTimeValue(
  typeof __SPEG_BUILD_CLERK_PUBLISHABLE_KEY__ === "undefined"
    ? undefined
    : __SPEG_BUILD_CLERK_PUBLISHABLE_KEY__,
);
export const buildTimeClerkCliOAuthClientId = readBuildTimeValue(
  typeof __SPEG_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__ === "undefined"
    ? undefined
    : __SPEG_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__,
);
export const buildTimeRelayClientTracing = {
  tracesUrl: readBuildTimeValue(
    typeof __SPEG_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__ === "undefined"
      ? undefined
      : __SPEG_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__,
  ),
  tracesDataset: readBuildTimeValue(
    typeof __SPEG_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__ === "undefined"
      ? undefined
      : __SPEG_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__,
  ),
  tracesToken: readBuildTimeValue(
    typeof __SPEG_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__ === "undefined"
      ? undefined
      : __SPEG_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__,
  ),
} as const;
...
export function resolveRelayClientTracingConfig(
  ...
  const tracesUrl = env.SPEG_RELAY_CLIENT_OTLP_TRACES_URL?.trim() || fallback.tracesUrl;
  const tracesDataset =
    env.SPEG_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() || fallback.tracesDataset;
  const tracesToken = env.SPEG_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() || fallback.tracesToken;
...
export function makeRelayUrlConfig(fallback = buildTimeRelayUrl) {
  const runtimeConfig = Config.nonEmptyString("SPEG_RELAY_URL");
...
export const hasCloudPublicConfig = Boolean(
  (normalizeSecureRelayUrl(process.env.SPEG_RELAY_URL ?? "") ?? buildTimeRelayUrl) &&
  (process.env.SPEG_CLERK_PUBLISHABLE_KEY?.trim() || buildTimeClerkPublishableKey) &&
  (process.env.SPEG_CLERK_CLI_OAUTH_CLIENT_ID?.trim() || buildTimeClerkCliOAuthClientId),
);
