...
import {
  ...
  signRelayJwt,
  verifyRelayJwt,
} from "@speg/shared/relayJwt";
import { isSecureRelayUrl } from "@speg/shared/relayUrl";
  ...
  Effect.mapError(
    () =>
      new EnvironmentHttpInternalServerError({
        message: "SPEG_RELAY_URL must be configured as a secure absolute HTTPS origin.",
      }),
  ),
);
...
const makeCloudLinkProof = Effect.fn("environment.cloud.makeLinkProof")(function* (
  ...
  const payload = {
    iss: `speg-env:${descriptor.environmentId}`,
...
const relayClientRequest = <A>(
  ...
  HttpClientRequest.post(input.url).pipe(
    ...
    Effect.mapError(
      (cause) =>
        new EnvironmentHttpInternalServerError({
          message: `SPEG Connect relay request failed: ${String(cause)}`,
        }),
    ),
...
const reconcileDesiredCloudLinkWith = Effect.fn("environment.cloud.reconcileDesiredLinkWith")(
          ...
          onNone: () =>
            Effect.fail(
              new EnvironmentHttpUnauthorizedError({
                message: "Run `speg connect link` to authorize this environment.",
              }),
            ),
  ...
  Effect.catchIf(
    ServerSecretStore.isSecretStoreError,
    failEnvironmentCloudInternalError("Could not persist desired SPEG Connect link state."),
  ),
  Effect.catchTags({
...
// explicit launcher stop (`speg service uninstall`, `systemctl stop`) during
...
export const releaseManagedTunnelOnShutdown = Effect.fn(
  ...
  // persisted URL: SPEG_RELAY_URL may have changed since the link was made.
...
const cloudEnvironmentHealthHandler = Effect.fn("environment.cloud.health")(
  function* (dependencies: CloudHttpDependencies, request: RelayCloudEnvironmentHealthRequest) {
    ...
    const proofOption = yield* verifyRelayJwt({
      ...
      audience: `speg-env:${environmentId}`,
...
export const connectHttpApiLayer = HttpApiBuilder.group(
  ...
  Effect.fnUntraced(function* (handlers) {
    ...
    return handlers
      ...
      .handle("spegMintCredential", ({ payload }) =>
