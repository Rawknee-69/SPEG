import { HostProcessExecutablePath, HostProcessPlatform } from "@speg/shared/hostProcess";
...
const BOOT_SERVICE_NAME = "speg";
export const BOOT_SERVICE_UNIT_FILE = `${BOOT_SERVICE_NAME}.service`;
export const BOOT_SERVICE_UNIT_ENV = "SPEG_BOOT_SERVICE_UNIT";
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=SPEG_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the SPEG background service.";
  }
}
            ...
            Effect.mapError(
              (cause) =>
                new PinnedRuntimeInstallError({
                  step: "verifying the pinned speg runtime",
                  cause,
                }),
            ),
