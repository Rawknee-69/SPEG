...
import { ThreadId } from "@speg/contracts";
import { PROJECT_FAVICON_FALLBACK_MARKER } from "@speg/shared/projectFavicon";
...
describe("AssetAccess", () => {
  ...
  it.effect("rejects workspace files outside the authorized root", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-asset-root-",
      });
      const outside = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-asset-outside-",
      });
  ...
  it.effect("preserves non-missing canonical path failures when issuing asset URLs", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-asset-permission-root-",
      });
      const htmlPath = path.join(root, "report.html");
  ...
  it.effect("issues exact workspace URLs for image previews", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-asset-image-workspace-",
      });
  ...
  it.effect("buckets project favicon expiry after content hashing", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-asset-favicon-expiry-",
      });
  ...
  it.effect("preserves structured project favicon resolution causes", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-asset-favicon-error-",
      });
