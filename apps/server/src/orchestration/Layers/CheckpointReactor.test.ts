...
function createGitRepository() {
  const cwd = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "speg-checkpoint-handler-"));
...
describe("CheckpointReactor", () => {
  ...
  async function createHarness(options?: {
    ...
    const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
      prefix: "speg-checkpoint-reactor-test-",
    });
  ...
  it("adopts a drifted checkout as the thread branch on a dedicated worktree", async () => {
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "speg/original-branch",
      localStatusRefName: "speg/renamed-by-agent",
    });
    ...
    await waitForEvent(
      ...
      (event) =>
        ...
        (event as unknown as { payload: { branch?: string } }).payload.branch ===
          "speg/renamed-by-agent",
    );

    const snapshot = await harness.readModel();
    const thread = snapshot.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.branch).toBe("speg/renamed-by-agent");
  });
  ...
  it("does not adopt a drifted checkout when the worktree is shared by another thread", async () => {
    ...
    const snapshot = await harness.readModel();
    const thread = snapshot.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.branch).toBe("speg/original-branch");
  });
  ...
  it("does not adopt a temporary placeholder checkout as the thread branch", async () => {
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "speg/original-branch",
      localStatusRefName: "speg/0a1b2c3d",
    });
  ...
  it("continues processing runtime events after a single checkpoint runtime failure", async () => {
    const nonRepositorySessionCwd = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "speg-checkpoint-runtime-non-repo-"),
    );
