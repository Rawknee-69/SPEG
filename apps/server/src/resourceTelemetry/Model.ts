...
export interface TelemetryCounters {
  ...
  readonly allSpeg: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allSpeg: ResourceTelemetryAggregate;
  };
...
function applyLifecycleCounters(input: {
  ...
  let allSpeg = input.counters.allSpeg;
  for (const processDelta of input.deltas) {
    ...
    allSpeg = incrementCounters(allSpeg, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allSpeg = incrementCounters(allSpeg, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allSpeg = incrementCounters(allSpeg, { processExits: 1 });
  }

  return { backend, electron, monitor, allSpeg };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allSpeg: aggregate(ordered, counters.allSpeg),
    },
    deltas: processDeltas,
  };
}
