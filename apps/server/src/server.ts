import { EnvironmentHttpApi } from "@speg/contracts";
...
import { disableTailscaleServe, ensureTailscaleServe } from "@speg/tailscale";
import { forkParked, ServerActivation } from "./serverActivation.ts";

// Effect's default preemptive shutdown waits 20s before finalizing request scopes.
// SPEG's primary transport is long-lived WebSocket RPC, whose Effect scope finalizer
        ...
        yield* forkParked(
          Effect.gen(function* () {
            ...
            yield* reconcileDesiredCloudLink(`http://127.0.0.1:${address.port}`).pipe(
              ...
              Effect.tap(() => Effect.logInfo("SPEG Connect desired link reconciled on startup")),
              Effect.catch((cause) =>
                Effect.logWarning("Failed to reconcile SPEG Connect desired link on startup", {
                  cause,
                }),
              ),
            );
