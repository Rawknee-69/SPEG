...
import { sanitizeBranchFragment } from "@speg/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@speg/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("SPEG_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("SPEG_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("SPEG_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("SPEG_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("speg/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `speg/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set SPEG_BITBUCKET_EMAIL and SPEG_BITBUCKET_API_TOKEN, or SPEG_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
