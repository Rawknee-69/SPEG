...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@speg/contracts";
import { fromLenientJson } from "@speg/shared/schemaJson";
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".speg", "vcs.json");
