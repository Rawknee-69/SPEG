...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(SpegProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "speg.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("SpegProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid speg.json", () =>
      Effect.gen(function* () {
        const loader = yield* SpegProjectFileLoader.SpegProjectFileLoader;
    ...
    it.effect("returns none when speg.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* SpegProjectFileLoader.SpegProjectFileLoader;
        const cwd = yield* makeTempDir;
