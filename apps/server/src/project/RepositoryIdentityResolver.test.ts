...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-repository-identity-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:Speg/speg.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/speg/speg");
      expect(normalizeResolvedPath(resolvedIdentityRoot)).toBe(normalizeResolvedPath(resolvedCwd));
      expect(identity?.displayName).toBe("speg/speg");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("speg");
      expect(identity?.name).toBe("speg");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      const repoRoot = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-repository-identity-nested-root-test-",
      });
      ...
      yield* git(repoRoot, ["remote", "add", "origin", "git@github.com:Speg/speg.git"]);
  ...
  it.effect("returns null for non-git folders and repos without remotes", () =>
    Effect.gen(function* () {
      ...
      const nonGitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-repository-identity-non-git-",
      });
      const gitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-repository-identity-no-remote-",
      });
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-repository-identity-upstream-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/speg.git"]);
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:Speg/speg.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-repository-identity-nested-group-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@gitlab.com:Speg/platform/speg.git"]);

      const resolver = yield* RepositoryIdentityResolver.RepositoryIdentityResolver;
      const identity = yield* resolver.resolve(cwd);

      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/speg/platform/speg");
      expect(identity?.displayName).toBe("speg/platform/speg");
      expect(identity?.owner).toBe("speg");
      expect(identity?.name).toBe("speg");
    }).pipe(Effect.provide(RepositoryIdentityResolver.layer)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "speg-repository-identity-late-remote-test-",
        });
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/speg/speg");
        expect(refreshedIdentity?.name).toBe("speg");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "speg-repository-identity-remote-change-test-",
      });
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/speg/speg");

      yield* git(cwd, ["remote", "set-url", "origin", "git@github.com:Speg/speg-next.git"]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/speg/speg");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/speg/speg-next");
      expect(refreshedIdentity?.displayName).toBe("speg/speg-next");
      expect(refreshedIdentity?.name).toBe("speg-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),
