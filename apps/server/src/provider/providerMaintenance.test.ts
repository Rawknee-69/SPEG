...
import { ProviderDriverKind, ProviderInstanceId, type ServerProvider } from "@speg/contracts";
...
it.layer(NodeServices.layer)("providerMaintenance", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("speg-vite-plus-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("speg-bun-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("speg-pnpm-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("speg-native-package-tool-native-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("speg-scoped-package-tool-native-capabilities");
  ...
  it.effect("keeps npm updates for binaries symlinked into npm's global node_modules tree", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("speg-npm-capabilities");
  ...
  it.effect("uses Effect FileSystem realPath when detecting pnpm global symlinks", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("speg-pnpm-realpath-capabilities");
      const binDir = NodePath.join(tempDir, "bin");
      const packageBinDir = NodePath.join(
        tempDir,
        ".local",
