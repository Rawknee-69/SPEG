 ...
 * Enable with: SPEG_GROK_ACP_PROBE=1 bun run test GrokAcpCliProbe
 *
...
const makeProbeRuntime = Effect.gen(function* () {
  ...
  return yield* makeGrokAcpRuntime({
    ...
    childProcessSpawner,
    cwd: process.cwd(),
    clientInfo: { name: "speg-grok-probe", version: "0.0.0" },
  });
});

describe.runIf(process.env.SPEG_GROK_ACP_PROBE === "1")("Grok ACP CLI probe", () => {
