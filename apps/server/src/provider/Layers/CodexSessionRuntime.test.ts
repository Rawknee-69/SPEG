...
import { DEFAULT_MODEL, ThreadId } from "@speg/contracts";
...
describe("buildCodexDeveloperInstructions", () => {
  it("appends runtime info after the mode instructions", () => {
    ...
    NodeAssert.match(instructions, /SPEG/);
...
describe("SPEG browser developer instructions", () => {
  it("prefers the product-native preview tools in both collaboration modes", () => {
    for (const instructions of [
      CODEX_DEFAULT_MODE_DEVELOPER_INSTRUCTIONS,
      CODEX_PLAN_MODE_DEVELOPER_INSTRUCTIONS,
    ]) {
      NodeAssert.match(instructions, /speg-code/);
...
describe("hasConfiguredMcpServer", () => {
  it("detects inline Codex MCP configuration arguments", () => {
    ...
    NodeAssert.equal(
      hasConfiguredMcpServer(["-c", 'mcp_servers.speg-code.url="http://127.0.0.1/mcp"']),
      true,
    );
  });
});
  ...
  it("keeps launch args when explicit app-server args are provided", () => {
    NodeAssert.deepStrictEqual(
      codexSessionAppServerArgs(
        ["-c", "mcp_servers.speg-code.url=http://127.0.0.1/mcp"],
      ...
      [
        ...
        "foo",
        "-c",
        "mcp_servers.speg-code.url=http://127.0.0.1/mcp",
      ],
    );
  });
});
