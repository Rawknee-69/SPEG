...
import { deepMerge } from "@speg/shared/Struct";
import { createModelCapabilities } from "@speg/shared/model";
import { applyServerSettingsPatch } from "@speg/shared/serverSettings";
...
process.env.SPEG_CURSOR_ENABLED = "1";
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
            ...
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "speg-provider-registry-background-refresh-",
                }),
              ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "speg-provider-registry-merged-persist-",
                }),
              ),
          ...
          Effect.gen(function* () {
            ...
            const runtimeServices = yield* Layer.build(
              ProviderRegistryLive.pipe(
                ...
                Layer.provideMerge(
                  ServerConfig.layerTest(process.cwd(), {
                    prefix: "speg-provider-registry-opencode-authoritative-persist-",
                  }),
                ),
                Layer.provideMerge(NodeServices.layer),
              ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "speg-provider-registry-refresh-failure-",
                }),
              ),
              Layer.provideMerge(BackgroundPolicyAlwaysRunLayer),
              Layer.provideMerge(NodeServices.layer),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "speg-provider-registry-sync-failure-",
                }),
              ),
              Layer.provideMerge(BackgroundPolicyAlwaysRunLayer),
              Layer.provideMerge(NodeServices.layer),
      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          const missingBinary = `speg_codex_missing_`;
          const serverSettings = yield* makeMutableServerSettingsService(
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
                ...
                providerInstances: {
                  // Matches the shape the user had in `.speg/dev/settings.json`
      ...
      it.effect("re-probes when settings change the codex binaryPath", () =>
        Effect.gen(function* () {
          const firstMissing = `speg_codex_first_`;
          const secondMissing = `speg_codex_second_`;
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in SPEG settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in SPEG settings.");
        }),
      );
    });
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("runs Claude status probes with the configured CLAUDE_CONFIG_DIR", () => {
        const claudeConfigDir = "/tmp/speg-claude-home";
