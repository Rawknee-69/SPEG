...
import { compareSemverVersions } from "@speg/shared/semver";
import {
  buildServerProvider,
  nonEmptyTrimmed,
        ...
        probe: {
          ...
          message:
            openCodeSettings.serverUrl.trim().length > 0
              ? "OpenCode is disabled in SPEG settings. A server URL is configured."
              : "OpenCode is disabled in SPEG settings.",
        },
      });
    }
...
export const checkOpenCodeProviderStatus = Effect.fn("checkOpenCodeProviderStatus")(function* (
  ...
  if (!isExternalServer) {
    ...
    if (!version) {
      return fallback(
        new Error(
          `Unable to determine OpenCode version from \`opencode --version\` output. SPEG requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
        ),
        null,
      );
    }
