...
import { causeErrorTag } from "@speg/shared/observability";
    ...
    if (!cursorSettings.enabled) {
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "warning",
          auth: { status: "unknown" },
          message: "Cursor is disabled in SPEG settings.",
        },
      });
    }
  ...
  Effect.gen(function* () {
    ...
    const acpContext = yield* Layer.build(
      AcpSessionRuntime.layer({
        ...
        cwd: process.cwd(),
        clientInfo: { name: "speg-code-provider-probe", version: "0.0.0" },
...
function buildCursorCliCommandMissingMessage(binaryPath: string): string {
  return [
    ...
    `Install or enable the Cursor CLI, make sure \`${binaryPath}\` is on PATH, then restart SPEG.`,
    `See ${CURSOR_CLI_INSTALLATION_DOCS_URL}.`,
  ].join(" ");
}
