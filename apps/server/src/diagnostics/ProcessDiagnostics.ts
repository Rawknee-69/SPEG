...
>()("speg/diagnostics/ProcessDiagnostics") {}
    ...
    function* (input) {
      if (input.pid === process.pid) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some("Refusing to signal the SPEG server process."),
        };
      }
      ...
      if (!canSignalCategory(selected.category)) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some(`Process ${input.pid} is not a signalable SPEG backend descendant.`),
        };
      }
