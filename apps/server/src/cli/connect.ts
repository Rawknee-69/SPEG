...
import { RelayOkResponse } from "@speg/contracts/relay";
import * as RelayClient from "@speg/shared/relayClient";
import { withRelayClientTracing } from "@speg/shared/relayTracing";
...
const authorizeCli = Effect.fn("cloud.cli.authorize")(function* (options: {
  ...
  const existing = yield* tokens.getExisting.pipe(
    Effect.catchTag("CloudCliCredentialRefreshError", () =>
      Console.log(
        "The stored SPEG Connect credential could not be refreshed; signing in again.",
...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `speg connect link` to authorize and enable SPEG Connect."
    : !status.desired
      ? "Run `speg connect link` to enable SPEG Connect."
      : !status.linked
        ? "Start SPEG to provision the environment link and launch its managed tunnel."
        : undefined;
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.confirm({
      message: `The SPEG relay client is required for SPEG Connect. Download and install version ${version}?`,
      initial: false,
    }),
  );
...
const logCloudDisconnectFailure = (
  ...
  Effect.logWarning("SPEG Connect disconnect operation failed.").pipe(
    Effect.annotateLogs({
      operation,
      clearAuthorization,
...
export const reportCloudDisconnectResults = Effect.fn("cloud.cli.report_disconnect_results")(
  function* (input: {
    ...
    if (input.liveResult.status === "failed") {
      ...
      yield* Console.warn(
        "SPEG Connect is disabled, but the running server could not stop its tunnel.\nRestart that server to stop the connector.",
      );
    } else {
      yield* Console.log("SPEG Connect is disabled locally.");
    }
    ...
    if (Exit.isFailure(input.relayResult)) {
      ...
      yield* Console.warn(
        input.clearAuthorization
          ...
          : "Could not revoke the relay-side environment record yet.\nRun `speg connect unlink` again when the relay is reachable.",
      );
...
const disconnectCloud = Effect.fn("cloud.cli.disconnect")(function* (options: {
  ...
  if (options.clearAuthorization) {
    yield* Console.log(
      "Signed out of SPEG Connect locally.\nThe background service is managed separately with `speg service`.",
    );
  }
});
...
const linkEnvironmentForConnect = Effect.fn("cloud.cli.link_environment")(function* (options: {
  ...
  if (!publishOnly) {
    ...
    if (Option.isNone(installed)) {
      yield* Console.log("SPEG Connect setup cancelled. The relay client was not installed.");
      return null;
    }
...
const connectLoginCommand = Command.make("login", {
  ...
  Command.withDescription("Authorize the SPEG Connect CLI without enabling remote access."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log("SPEG Connect\n");
...
const connectLinkCommand = Command.make("link", {
  ...
  Command.withDescription("Authorize this environment for SPEG Connect on next start."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (linked) {
          ...
          yield* Console.log(
            flags.publishOnly
              ? `✓ Authorized${connectedAs(linked.identity)}\n\nNext\n  Start SPEG to publish agent activity (no managed tunnel).`
...
const connectStatusCommand = Command.make("status", {
  ...
  Command.withDescription("Show persisted SPEG Connect and relay client state."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        if (!enabled) {
          ...
          if (!linkedNow && (yield* CliState.readCliDesiredLinkMode) === "publish_only") {
            yield* CliState.setCliDesiredCloudLink(false);
            yield* Console.log("Cancelled the pending publish-only SPEG Connect link.");
          }
        ...
        // out of band without SPEG Connect.
        if (!(yield* tokens.hasCredential)) {
          yield* Console.log(
            "Run `speg connect login` first so this environment can be authorized to publish.",
          );
          return;
        }
        // A link may already be desired (e.g. `speg connect link` before the
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A SPEG Connect link is already pending. Start SPEG to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
        yield* CliState.setCliDesiredCloudLink(true, "publish_only");
        yield* Console.log(
          "Restart SPEG to finish authorizing this environment to publish (no managed tunnel is created).",
        );
      }),
    ),
  ),
...
const connectUnlinkCommand = Command.make("unlink", {
  ...
  Command.withDescription("Disable SPEG Connect while retaining the stored authorization."),
  Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
  ...
  Command.withDescription("Disable SPEG Connect and clear the stored CLI authorization."),
  Command.withHandler((flags) =>
...
export const connectCommand = Command.make("connect", {
  ...
  Command.withDescription("Set up SPEG Connect for this machine."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (background) {
          yield* Console.log(
            "\n✓ Background service ready\n\nSPEG will stay reachable after you log out.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
