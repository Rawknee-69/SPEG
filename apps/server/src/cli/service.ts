...
export function formatServiceStatus(
  ...
  if (!status.installed) {
    return "SPEG service\n  Status: not installed\n  Next: Run `speg service install`.";
  }
  return [
    "SPEG service",
    `  Status: ${status.current ? `installed · speg@${cliVersion}` : "needs an update or repair"}`,
    `  Unit: ${status.unitPath}`,
    `  Logs: ${status.logPath}`,
    ...(status.current ? [] : ["  Next: Run `npx speg@latest service update`."]),
  ].join("\n");
}
...
const serviceInstallCommand = Command.make("install", projectLocationFlags).pipe(
  Command.withDescription("Install SPEG as a background service for this user."),
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `SPEG service is already installed with speg@${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} SPEG service with speg@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
...
const serviceUpdateCommand = Command.make("update", projectLocationFlags).pipe(
  Command.withDescription(
    "Update or repair the background service using this CLI version. Use `npx speg@latest service update` for the latest release.",
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        const result = yield* reconcileService();
        if (!result.changed) {
          yield* Console.log(`SPEG service is already using speg@${packageJson.version}.`);
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} SPEG service with speg@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the SPEG background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed ? "Removed the SPEG service." : "SPEG service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the SPEG background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log("SPEG is already set up to run in the background on this machine.");
    return true;
  }
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ? "The installed SPEG service needs an update or repair. Update it now?"
        : "Run SPEG in the background whenever this machine boots? " +
          "It stays reachable through SPEG Connect even after you log out.",
      initial: true,
    }),
  );
  if (!wanted) {
...
export const serviceCommand = Command.make("service").pipe(
  Command.withDescription("Manage the SPEG background service."),
