...
import { HostProcessArguments } from "@speg/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `speg` command.
 */
...
/**
 * The `speg` package spec to suggest. The literal spec the user typed (e.g.
 * `speg@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "speg@nightly" : "speg";
}

/**
 * Render a `speg <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx speg connect` suggests
 * `npx speg serve`, a global install suggests `speg serve`, and a nightly build
